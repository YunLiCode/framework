        ...
        synchronized (mLock) {
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                if (simId == PhoneConstants.GEMINI_SIM_2) {
                    ...
                    mInstance2 = new UiccController(c, ci, simId);
                    return mInstance2;
                } else {
                    if (mInstance != null) {
                        throw new RuntimeException("UiccController.make() should only be called once");
                    }
                    mInstance = new UiccController(c, ci, simId);
                    return mInstance;
                }
            } else {
                if (mInstance != null) {
                            ...
                            switch (result[0]) {
                                ...
                                default:
                                    if (FeatureOption.MTK_GEMINI_SUPPORT) {
                                        ...
                                        int dualSimMode = Settings.Global.getInt(
                                                mContext.getContentResolver(),
                ...
                case EVENT_QUERY_ICCID_DONE_FOR_HOT_SWAP:
                    ...
                    if (!FeatureOption.MTK_GEMINI_SUPPORT) {
                        ...
                        if(enCryState == null || "".equals(enCryState) || "trigger_restart_framework".equals(enCryState)) {
                            ...
                            DefaultSIMSettings.onAllIccidQueryComplete(mContext, phone, iccid, null, false);
                        }
                    }
                    break;
...
///LEWA BEGIN        
        ...
        CharSequence detail = mContext.getText(com.mediatek.internal.R.string.sim_missing_detail);
