                    ...
                    if((ss.getIsManualSelection() == true) && (newSS.getIsManualSelection() == false)){
                        ...
                        if (FeatureOption.MTK_GEMINI_SUPPORT) {
                            if (phone instanceof GSMPhone){
                                GSMPhone peerPhone = ((GSMPhone)phone).getPeerPhone();
                                if(peerPhone != null){
                                    if(peerPhone.getServiceState().getIsManualSelection()== false){
                                        log("Both sim are NOT in manual selection mode,stop reminder service");
                                        Intent sIntent = new Intent();
                                        sIntent.setClassName("com.android.phone","com.mediatek.settings.NoNetworkPopUpService");
    ...
    private void pollState() {
        ...
        if (radioState == CommandsInterface.RadioState.SIM_LOCKED_OR_ABSENT) {
            ...
            if ((airplaneMode == 1) || (isPowerOff ==1)) {
                ...
                boolean hasPeerSIMInserted = FeatureOption.MTK_GEMINI_SUPPORT ? phone.getPeerPhone().isSimInsert() : false;
                boolean hasSIMInserted = phone.isSimInsert();
                 //this needs to be modified for Gemini+
...
//MTK-START [mtk03851][111124]
    ...
    public void setGprsConnType(int type) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT && type == 1 &&
                !PhoneFactory.isDualTalkMode()) {
            GeminiPhone mGeminiPhone = (GeminiPhone)PhoneFactory.getDefaultPhone();
            if (mGeminiPhone != null &&
                    !mGeminiPhone.isGprsDetached(phone.getPeerPhone().getMySimId())) {
                log("setGprsConnType: Peer phone is not detached, skip set to always!!!");
                return;
            }
        }
