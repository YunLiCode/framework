...
public final class AdnRecordCache extends Handler implements IccConstants {
     ...
     */
    public synchronized void updateAdnByIndex(int efid, AdnRecord adn, int recordIndex,
            String pin2,
            Message response) {
    ...
    /**
     * Replace oldAdn with newAdn in ADN-like record in EF The ADN-like records
     * must be read through requestLoadAllAdnLike() before
     * 
     * @param efid must be one of EF_ADN, EF_FDN, and EF_SDN
     * @param oldAdn is the adn to be replaced If oldAdn.isEmpty() is ture, it
     *            insert the newAdn
     * @param newAdn is the adn to be stored If newAdn.isEmpty() is true, it
     *            delete the oldAdn
     * @param pin2 is required to update EF_FDN, otherwise must be null
     * @param response message to be posted when done response.exception hold
     *            the exception in error
