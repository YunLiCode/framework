...
///LEWA BEGIN
        if(checkPermission(sentIntent, new String[]{destAddr}))
///LEWA END
...
///LEWA BEGIN
        if(checkPermission(sentIntent, new String[]{destAddr, text}))
///LEWA END
...
///LEWA BEGIN
        if(checkPermission(sentIntents, new String[]{destAddr}))
///LEWA END
...
///LEWA BEGIN
    private boolean checkPermission(List<PendingIntent> sentIntents, String[] args){
        if(lewa.content.PermissionHelper.checkPermission(mPhone.getContext(), getCallingPid(), getCallingUid(), lewa.content.PermissionHelper.PERMISSION_SEND_SMS, args)){
            return true;
        } else {
            try{
                for(PendingIntent i : sentIntents)
                    i.send(-1);
            }catch(Exception e){}
            return false;
        }
    }
    private boolean checkPermission(PendingIntent sentIntent, String[] args){
        if(lewa.content.PermissionHelper.checkPermission(mPhone.getContext(), getCallingPid(), getCallingUid(), lewa.content.PermissionHelper.PERMISSION_SEND_SMS, args)){
            return true;
        } else {
            try{
