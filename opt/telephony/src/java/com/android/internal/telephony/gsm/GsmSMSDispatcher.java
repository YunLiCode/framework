...
public final class GsmSMSDispatcher extends SMSDispatcher {
    ...
    private void handleBroadcastSms(AsyncResult ar) {
        try {
            ...
            dispatchBroadcastPdus(pdus, header.isEmergencyMessage());
    ...
    private void handleEtwsPrimaryNotification(AsyncResult ar) {
        ...
        if(!isDuplicated) {
            ...
            // broadcast it to app
            handleEtwsPdu(noti.getEtwsPdu(), noti.plmnId);
            stopEtwsAlarm();
            startEtwsAlarm();
        } else {
    ...
    private void handleEtwsPdu(byte[] pdu, String plmn) {
        if(pdu == null || pdu.length != EtwsUtils.ETWS_PDU_LENGTH) {
            Log.d(TAG, "invalid ETWS PDU");
            return;
        }
