...
import com.android.internal.telephony.gsm.PBEntry;
import com.android.internal.telephony.gsm.PBMemStorage;
...
public final class RIL extends BaseCommands implements CommandsInterface {
    ...
    private boolean dtmfStatus = DTMF_STATUS_STOP;
    ...
    class RILReceiver implements Runnable {
        ...
        run() {
            int retryCount = 0;
            String socketRil = SOCKET_NAME_RIL_1;

            if (mySimId == PhoneConstants.GEMINI_SIM_2) {
                socketRil = SOCKET_NAME_RIL_2;
            }
...
/* 3G switch start */
                    if (FeatureOption.MTK_GEMINI_3G_SWITCH) {
                        if (SystemProperties.getInt("gsm.3gswitch", 1) == 2) {
                            if (socketRil == SOCKET_NAME_RIL_1)
                                socketRil = SOCKET_NAME_RIL_2;
                            else
                                socketRil = SOCKET_NAME_RIL_1;
...
/* 3G switch end */
    ...
    startDtmf(char c, Message result) {
        /// M: Solve [ALPS00334963][Rose][6577JB][Free Test][Network]The SIM card network lost and the information of SIM card shows as "No service" in the unlock screen when we input many numbers ceaselessly until the call end automatically(3/3). @{
        /* DTMF request will be ignored when the count of requests reaches 32 */
        if (dtmfRequestCount < MAXIMUM_DTMF_REQUEST) {
            //MTK-START [mtk08470][130121][ALPS00445268] Block non-sequence DTMF input
            if (dtmfStatus == DTMF_STATUS_STOP) {
           RILRequest rr = RILRequest.obtain(RIL_REQUEST_DTMF_START, result);
                if (RILJ_LOGD) riljLog(rr.serialString() + "> " + requestToString(rr.mRequest) + " " + dtmfRequestCount);
           rr.mp.writeString(Character.toString(c));
           send(rr);
           dtmfRequestCount++;
                dtmfStatus = DTMF_STATUS_START;
            }
            else {
                riljLog("DTMF status conflict, want to start DTMF when status is " + dtmfStatus);	
            }
            //MTK-END [mtk08470][130121][ALPS00446908]MTK added
        }
        else {
           riljLog("DTMF requests are over " + MAXIMUM_DTMF_REQUEST);	
        }
        /// @}
    }
    ...
    processSolicited (Parcel p) {
        ...
        if ((rr.mRequest == RIL_REQUEST_DTMF_START) || 
            ...
            dtmfRequestCount = (dtmfRequestCount > 0) ? dtmfRequestCount - 1 : 0;
        }
        /// @}
...
*/
    ...
    */
    ...
    private Object responseSimSmsMemoryStatus(Parcel p) {
        SmsMemoryStatus response;

        response = new SmsMemoryStatus();
