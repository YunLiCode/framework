...
//#if 0  //debug: disable prepare
		...
		// TODO: RTSPEarlyEndTimeUs's not porting yet.
		    int64_t durationUs;
		    if (mDataSourceType == SOURCE_Rtsp && mSource != NULL && 
			mSource->getDuration(&durationUs) == OK && 
			durationUs == 0) {
			ALOGE("RTSP play end before duration %lld",
				durationUs);
...
status_t NuPlayer::instantiateDecoder(bool audio, sp<Decoder> *decoder) {
    ...
    if(!audio)
    {
        ...
        //[4.2migration]TODO: modify convertMetaDataToMessage
        //    meta->findInt64(kKeyHLSVideoFirestRender, &mVideoFirstRenderTimestamp);
        //    ALOGE("mVideoFirstRenderTimestamp is %.2f sec, width = %d, height = %d", mVideoFirstRenderTimestamp / 1E6, mVideoWidth, mVideoHeight); 
    }
