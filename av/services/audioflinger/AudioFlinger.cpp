...
if (mType == MIXER) {
    ...
    // SRS Processing
#endif
    if(mOutDevice == AUDIO_DEVICE_OUT_SPEAKER)
    {
        ALOGV("mAudioFlinger->SetFsync()");
        gFSync.setFsync();
    }
#endif
...
#ifdef MTK_AUDIO
                            || (!mActiveTracks.size()&&gFSync.underflow()&& (mOutDevice ==AUDIO_DEVICE_OUT_SPEAKER))
#endif
            ) {
                if (!mStandby) {
...
void AudioFlinger::MixerThread::threadLoop_standby()
{
    ...
    if (mFastMixer != NULL) {
      ...
      if(mDumpthread != NULL)
      {
          ALOGD("delete mDumpthread");
          mDumpthread->Stop ();
          if (mDumpthread.get() )
          {
              int ret = mDumpthread->requestExitAndWait();
              if (ret == WOULD_BLOCK) {
                  mDumpthread->requestExit();
                  }
              mDumpthread.clear();
              mDumpthread = NULL;
          }
...
void AudioFlinger::MixerThread::threadLoop_mix()
{
    ...
    char value[PROPERTY_VALUE_MAX];
    int ret;
    property_get(gaf_mixer_propty, value, "0");
    int bflag=atoi(value);
    if(bflag)
    {
        if(mDumpthread == NULL)
        {
            ALOGD("new mDumpthread ");
            mDumpthread = new AudioDumpThread("AudioMixerDump");
            if (mDumpthread.get()) {
                mDumpthread->run();
            }
        }
        if(mDumpthread != NULL)
        {
            mDumpthread->dump(gaf_mixer_pcm,mMixBuffer,mixBufferSize,gaf_mixer_propty);
        }
    }
#endif
#endif
    ...
    for (size_t i=0 ; i<count ; i++) {
                ...
                !track->isPaused() && !track->isTerminated())
        {
            ...
            if ((track->sharedBuffer() != 0) || track->isTerminated() ||
                ...
                ALOGD("BUFFER TIMEOUT:   track->mUnderrunCount++;(%d)  track->mRetryCount(%d)",track->mUnderrunCount++,track->mRetryCount);
                #ifdef MTK_AUDIO
                if(track->mRetryCount <= 42)
                {
                    char value[PROPERTY_VALUE_MAX];
                    int result;
                    ...
                    if(result){
                        aee_system_warning("Audioflinger", NULL, DB_OPT_FTRACE,"Audioflinger track is underrun");
                        track->mRetryCount = kMaxTrackRetries;
                    }
                }
                #endif
...
#ifdef MTK_AUDIO
#ifdef DEBUG_AUDIO_PCM
        if(mTrackDumpthread != NULL)
        {
            mTrackDumpthread->Stop ();
            if (mTrackDumpthread.get() )
            {
                int ret = mTrackDumpthread->requestExitAndWait();
                if (ret == WOULD_BLOCK) {
                    mTrackDumpthread->requestExit();
                }
                mTrackDumpthread.clear();
                mTrackDumpthread = NULL;
            }
        }
#endif
#endif
        ...
        AudioBufferProvider::Buffer* buffer, int64_t pts)
{
    ...
    if (CC_LIKELY(framesReady)) {
        ...
        char value[PROPERTY_VALUE_MAX];
        int ret;
        property_get(gaf_track_propty, value, "0");
        int bflag=atoi(value);
        if(bflag)
        {
            if(mTrackDumpthread == NULL)
            {
                ALOGD("new mTrackDumpthread ");
                mTrackDumpthread = new AudioDumpThread("AudioTrackDump");
                if (mTrackDumpthread.get()) {
                    mTrackDumpthread->run();
                }
            }
            if(mTrackDumpthread != NULL)
            {
                mTrackDumpthread->dump(fileName,buffer->raw,buffer->frameCount*cblk->frameSize,gaf_track_propty);
            }
        }
...
void AudioFlinger::PlaybackThread::Track::stop()
{
    ...
    if (thread != 0) {
        ...
        if(mTrackDumpthread != NULL)
        {
            ALOGD("PlaybackThread mDumpthread stop");
            mTrackDumpthread->Stop ();
            if (mTrackDumpthread.get() )
            {
                int ret = mTrackDumpthread->requestExitAndWait();
...
void AudioFlinger::PlaybackThread::Track::pause()
{
    ...
    if (thread != 0) {
        ...
        if(mTrackDumpthread != NULL)
        {
            ALOGD("PlaybackThread mDumpthread pause");
            mTrackDumpthread->Stop ();
            if (mTrackDumpthread.get() )
            {
                int ret = mTrackDumpthread->requestExitAndWait();
