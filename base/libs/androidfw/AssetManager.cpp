...
///LEWA BEGIN
static const char* kLewaAssets = "framework/lewa-res.apk";
///LEWA END
...
///LEWA BEGIN
        ...
        ALOGV("Looking for asset '%s' in '%s'\n",
                assetName.string(), /* mAssetPaths.itemAt(i) */ ap.path.string());
        Asset* pAsset = openNonAssetInPathLocked(assetName.string(), mode, ap /* mAssetPaths.itemAt(i) */);
///LEWA END
        if (pAsset != NULL) {
...
///LEWA BEGIN
        ...
        ALOGV("Looking for non-asset '%s' in '%s'\n", fileName, /* mAssetPaths.itemAt(i) */ ap.path.string());
        Asset* pAsset = openNonAssetInPathLocked(
            fileName, mode, ap /* mAssetPaths.itemAt(i) */);
///LEWA END
        if (pAsset != NULL) {
            return pAsset != kExcludedAsset ? pAsset : NULL;
        }
...
void AssetManager::updateResTableFromAssetPath(ResTable *rt, const asset_path& ap, void *cookie) const
{
    ...
    ALOGV("Looking for resource asset '%d' in '%s'\n", cookiePos, ap.path.string());
    if (ap.type != kFileTypeDirectory) {
        if (/* i == 0 */ cookiePos == 1 || cookiePos == 2) {
        ...
        if (sharedRes == NULL) {
            ...
            if (/* i == 0 */ (cookiePos == 1) && ass != NULL) {
                // If this is the first resource table in the asset
                // manager, then we are going to cache it so that we
                // can quickly copy it out for others.
                ALOGV("Creating shared resources for %s", ap.path.string());
                sharedRes = new ResTable();
                sharedRes->add(ass, /* (void*)(i+1) */ cookie, false /* , idmap */);
                sharedRes = const_cast<AssetManager*>(this)->
    ...
    if ((ass != NULL || sharedRes != NULL) && ass != kExcludedAsset) {
        ...
        if (sharedRes != NULL) {
            ...
            rt->add(ass, /* (void*)(i+1) */ cookie, !shared /* , idmap */);
        }
