...
///LEWA BEGIN
#define LEWA_BOOTANIMATION_FILE "/data/system/face/bootanimation.zip"
///LEWA END
...
/*
    ...
    //add print log
*/	
    // @add default blackground for RGB8888 to fix green line issue 
    const size_t kLength = tw * th * 4;
    unsigned char *background = new unsigned char[kLength];
    unsigned char *addr = background;
    memset(background, 0, kLength);
    ...
    switch (bitmap.getConfig()) {
        case SkBitmap::kARGB_8888_Config:
            ...
            if (tw != w || th != h) {
                glTexImage2D(GL_TEXTURE_2D, 0, GL_BGRA, tw, th, 0, GL_BGRA,
                        GL_UNSIGNED_BYTE, (void *)background);
                //glTexImage2D(GL_TEXTURE_2D, 0, GL_BGRA, tw, th, 0, GL_BGRA,
                //        GL_UNSIGNED_BYTE, 0);
                glTexSubImage2D(GL_TEXTURE_2D, 0,
                        0, 0, w, h, GL_BGRA, GL_UNSIGNED_BYTE, p);
...
status_t BootAnimation::readyToRun() {
    ...
    XLOGD("draw a black screen first to avoid landscape ghost image");
    ...
    if (bBootOrShutDown) {
///LEWA BEGIN
        status_t err = mZip.open(LEWA_BOOTANIMATION_FILE);
        if (err != NO_ERROR) {
