...
/// LEWA BEGIN
    private ImageView flashlightCover;
    private BroadcastReceiver mTorchStateReceiver = new BroadcastReceiver() {
        public void onReceive(Context paramAnonymousContext, Intent paramAnonymousIntent) {
            final boolean bool = paramAnonymousIntent.getBooleanExtra("IS_ENABLE", false);
            post(new Runnable() {
                public void run(){
                updateTorchCover(bool);
            }
       });
    }
  };

    private void updateTorchCover(boolean b) {
        if(flashlightCover == null) {
            flashlightCover = new ImageView(mContext);

            flashlightCover.setLayoutParams(new FrameLayout.LayoutParams(LayoutParams.MATCH_PARENT,LayoutParams.MATCH_PARENT));
        }
        if(b) {
            flashlightCover.setImageResource(com.lewa.internal.R.drawable.flashlight_background_sos_on);
        }
        if ((b) && (flashlightCover.getParent() == null)) {
            addView(flashlightCover);
            flashlightCover.setOnClickListener(new View.OnClickListener() {
                public void onClick(View v) {
                    if(flashlightCover.getParent() != null)
                        removeView(flashlightCover);
                    Intent i = new Intent("com.lewa.flashlight.TOGGLE_FLASHLIGHT");
                    mContext.sendBroadcast(i);
                }
            });
        }

        else if(flashlightCover.getParent() != null)
           removeView(flashlightCover);
    }
/// LEWA END
...
///LEWA BEGIN
        IntentFilter localIntentFilter = new IntentFilter("com.lewa.flashlight.lockscreen");
        localIntentFilter.setPriority(1000);
        mContext.registerReceiver(mTorchStateReceiver, localIntentFilter);
///LEWA END
...
///LEWA BEGIN
        mContext.unregisterReceiver(mTorchStateReceiver);
///LEWA END
...
///LEWA BEGIN
    ...
    KeyguardSecurityView getCurrentKeyguardSecurityView(){
        return mCurrentKeyguardSecurityView;
    }
///LEWA END
...
///LEWA END
    ...
    private boolean isLockScreenPriorSimPin(SecurityMode mode) {
        ...
        if (mode == SecurityMode.SimPinPuk1 || mode == SecurityMode.SimPinPuk2) {
            bPinRequired = true;
        }
        ...
        public boolean onClickHandler(final View view,
            ...
            if (pendingIntent.isActivity()) {
                ...
                if (mViewStateManager.isChallengeShowing()) {
                    ...
                    /// M: Call dismiss instead of direct referece to mCallback
                    dimss();
                }
                return true;
            } else {
...
///LEWA END
    ...
    private Constructor<?> mLewaLockScreenConstructor;
...
///LEWA BEGIN
        if(view == null && securityMode == SecurityMode.None && KeyguardUtils.isLewaLockscreen(mContext)) {
            try {
                Context pkgContext = mContext.createPackageContext("com.lewa.lockscreen", Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
                if (mLewaLockScreenConstructor == null) {
                    Class<?> lewaLockScreen = pkgContext.getClassLoader().loadClass("com.android.internal.policy.impl.keyguard.FancyLockScreen");
                    mLewaLockScreenConstructor = lewaLockScreen.getConstructor(Context.class);
                }
                View v = (View)mLewaLockScreenConstructor.newInstance(pkgContext);
                v.setId(R.id.keyguard_selector_view);
...
///LEWA END
        ...
        if (view == null && layoutId != 0) {
            ...
            if (view instanceof KeyguardSimPinPukView) {
                ...
                if (securityMode == SecurityMode.SimPinPuk1) {
                    pinPukView.setSimId(PhoneConstants.GEMINI_SIM_1);
                } else if (securityMode == SecurityMode.SimPinPuk2) {
    ...
    private void showSecurityScreen(SecurityMode securityMode) {
        ...
        if (KeyguardUtils.isGemini()) {
            isSimOrAccount = isSimOrAccount || (securityMode == SecurityMode.SimPinPuk1
                    || securityMode == SecurityMode.SimPinPuk2);
        } else {
            isSimOrAccount = isSimOrAccount || (securityMode == SecurityMode.SimPinPuk1);
        }
        /// @}
        ...
        if (mSlidingChallengeLayout != null) {
            /// M: If dm lock is on, we should also set changelayout interactive
            mSlidingChallengeLayout.setChallengeInteractive(!fullScreenEnabled || dmLocked);
        }
...
///LEWA END
    ...
    private int getSecurityViewIdForMode(SecurityMode securityMode) {
        switch (securityMode) {
            ...
            case SimPinPuk1: return com.mediatek.internal.R.id.keyguard_sim_pin_puk_view;
            case SimPinPuk2: return com.mediatek.internal.R.id.keyguard_sim_pin_puk_view;
            /// @}
    ...
    private int getLayoutIdFor(SecurityMode securityMode) {
        switch (securityMode) {
            ...
            case SimPinPuk1: return com.mediatek.internal.R.layout.keyguard_sim_pin_puk_view;
            case SimPinPuk2: return com.mediatek.internal.R.layout.keyguard_sim_pin_puk_view;
            /// @}
...
///LEWA BEGIN
            try {
                Class<?> clazz = mCurrentKeyguardSecurityView.getClass();
                if(clazz.getSimpleName().equals("FancyLockScreen")) {
                    clazz.getMethod("cleanUp").invoke(mCurrentKeyguardSecurityView);
                }
            } catch (Exception e) {
            }
///LEWA END
    ...
    /// M: To avoid mOnClickListener hold mCallback, we exract the mCallback to a single function
    private void dimss() {
        mCallback.dismiss(false);
    }
}
