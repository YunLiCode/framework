...
import lewa.widget.MagnifierPopupWindow;
///LEWA END
...
///LEWA BEGIN
    //BEGIN:Added by wangqiang at 2013-09-02:add view with night mode
    ...
    private static View mNightView02;
    private static View mNightView03;
    int mNightModeValue = 0;//nightmode color default value
    int mNightmodeEnable = 0;//nightmode enable defalut value
    //END:Added by wangqiang at 2013-09-02:add view with night mode
///LEWA END
...
///LEWA BEGIN
    //BEGIN:Added by wangqiang at 2013-09-07
    private boolean mDisableProximitor;
    private LewaScreenOnProximityLock mProximitySensor;
    private boolean releaseScreenOnProximitySensor() {
        if (!mSystemReady)
            return false;
        mDisableProximitor = false;
        if (mProximitySensor != null)
        return mProximitySensor.release();
        return false;
    }
    //END:Added by wangqiang at 2013-09-07
///LEWA END
...
///LEWA BEGIN
    ...
    BroadcastReceiver mShowMagnifierReceiver=new BroadcastReceiver() {
        ...
        {
            Log.i("wangliqiang", "RECEIVER BROADCAST android.intent.action.SHOW_MAGNIFIER"+"mDownx: "+mDownX+"mDownY:"+mDownY);
            mShowMagnifier = true;
            if(mIsTouchDown)
            {
                Injector.getMagnifier(PhoneWindowManager.this).updateCache();
                Injector.getMagnifier(PhoneWindowManager.this).showMagnifier(mDownX, mDownY);
            }
        }
    };
    //LEWA ADD END
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(Settings.System.getUriFor(
                    ...
                    UserHandle.USER_ALL);
///LEWA BEGIN
        //BEGIN:Added by wangqiang at 2013-09-02:add listener for view with night mode
            resolver.registerContentObserver(Settings.System.getUriFor(
                    Settings.System.NIGHT_MODES), false, this);
            resolver.registerContentObserver(Settings.System.getUriFor(
                    "nightmode_enable"), false, this);
            resolver.registerContentObserver(Settings.System.getUriFor(
                    "inadvertmode_enable"), false, this);
        //END:Added by wangqiang at 2013-09-02:add listener for view with night mode
///LEWA END
            updateSettings();
        }
    ...
    static final int LONG_PRESS_MENU_CUSTOM_ACTION = 2;

    // What we do when the user long presses on menu
    private int mLongPressOnMenuBehavior = -1;
    private String mLongPressOnMenuAction;

    private void handleLongPressOnMenu() {
        if (mLongPressOnMenuBehavior < 0) {
            try {
                mLongPressOnMenuBehavior = mContext.getResources()
                        .getInteger(com.lewa.internal.R.integer.config_longPressOnMenuBehavior);
            } catch (Exception e) {
                // mLongPressOnMenuBehavior = -1;
                mLongPressOnMenuBehavior = 0;
            }
            if (mLongPressOnMenuBehavior < 0 /* && (mContext.getResources().getInteger( */
            ...
            if (mLongPressOnMenuBehavior == LONG_PRESS_MENU_CUSTOM_ACTION) {
                mLongPressOnMenuAction = mContext.getResources()
                        .getString(com.lewa.internal.R.string.config_longPressOnMenuAction);
            }
        }

        switch (mLongPressOnMenuBehavior) {
            case LONG_PRESS_MENU_SEARCH :
                /*
                 * performHapticFeedbackLw(null, HapticFeedbackConstants.LONG_PRESS, false);
                 * triggerVirtualKeypress(KeyEvent.KEYCODE_SEARCH);
                 */
                break;
            case LONG_PRESS_MENU_CUSTOM_ACTION :
                performHapticFeedbackLw(null, HapticFeedbackConstants.LONG_PRESS, false);
                try {
                    ...
                    mContext.startActivity((new Intent(
                            mLongPressOnMenuAction)).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                break;
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        keyDispatchFilter.addAction(IPO_ENABLE);
        keyDispatchFilter.addAction(IPO_DISABLE);
...
///LEWA BEGIN
        Injector.RegisteMagnifierReceiver(context,PhoneWindowManager.this); 
    }

//BEGIN:Added by wangqiang 2013-09-02:add view with night mode
    private void initNightView() {
        int i = 20;
        mNightWindowManager = ((WindowManager)mContext.getSystemService("window"));
        ...
        if (mNightView == null) {
            ...
            mNightView.setBackgroundColor(Color.argb(210 - i, 
                    Color.red(-16777216), Color.green(-16777216), Color.blue(-16777216)));
        }
        if (mNightView02 == null) {
            mNightView02 = new View(mContext);
            mNightView02.setFocusable(false);
            mNightView02.setFocusableInTouchMode(false);
            mNightView02.setBackgroundColor(0x4FFF0000);
        }
        if (mNightView03 == null) {
            mNightView03 = new View(mContext);
            mNightView03.setFocusable(false);
            mNightView03.setFocusableInTouchMode(false);
            mNightView03.setBackgroundColor(0x4FCDCD00);
        }
    }
    ...
    private boolean addNightView() {
        ...
        try {
            if (value == 0) {
                mNightWindowManager.addView(mNightView, mNightWindowParams);
                currentView = mNightView;
            } else if (value == 1) {
                mNightWindowManager.addView(mNightView02, mNightWindowParams);
                currentView = mNightView02;
            } else if (value == 2) {
                mNightWindowManager.addView(mNightView03, mNightWindowParams);
                currentView = mNightView03;
            }
            return true;
        } catch (Exception localException) {
            //Log.e("NightModeButton", "Can't add view : " + localException);
        }
        return false;
    }
    ...
    private void removeNightView(View view) {
        try {
            mNightWindowManager.removeViewImmediate(view);
        } catch (Exception localException) {
        }
    }
    ...
    private boolean removeNightView() {
        try {
            int value = getStateValue();
            if (value == 0) {
                mNightWindowManager.removeViewImmediate(mNightView);
                currentView = null;
            } else if (value == 1) {
                mNightWindowManager.removeViewImmediate(mNightView02);
                currentView = null;
            } else if (value == 2) {
                mNightWindowManager.removeViewImmediate(mNightView03);
                currentView = null;
            }
            return true;
        } catch (Exception localException) {
            //Log.e("NightModeButton", "Can't remove view : " + localException);
        }
        return false;
    }
    ...
    private void updateNightMode() {
        ...
        if (enable) {
            if (currentView != null) {
                Log.i(TAG, "updateNightMode 11111111");
                removeNightView(currentView);
            }
            Log.i(TAG, "updateNightMode 2222222");
            addNightView();
        } else {
            Log.i(TAG, "updateNightMode 3333333");
            removeNightView();
        }
    }

    private boolean getInadvertEnable() {
        return Settings.System.getInt(mContext.getContentResolver()
                , "inadvertmode_enable", 0) == 1 ? true : false;
    }
//END:Added by wangqiang 2013-09-02:add view with night mode
///LEWA END
...
///LEWA BEGIN
            ...
            if (mSystemReady) {
                ...
                if (mNightModeValue != value || mNightmodeEnable != enable) {
                    ...
                    mHandler.sendEmptyMessage(MSG_CREATE_NIGHTVIEW);
                }
            }
            //END:Added by wangqiang at 2013-09-02:add view with night mode
///LEWA END
...
///LEWA BEGIN
        if((WindowManager.LayoutParams.PRIVATE_FLAG_SHOW_DESKTOP & attrs.privateFlags) != 0)
            return false;
///LEWA END
...
///LEWA END
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            // LEWA BEGIN
            // add by zhangxianjia for torchlight
            if (up && keyguardOn) {
                setTorchOn(false);
            }
            // LEWA END
            ...
            // LEWA BEGIN
            // add by zhangxianjia for torchlight
            if ((event.getFlags() & KeyEvent.FLAG_LONG_PRESS) != 0 && keyguardOn && repeatCount != 0) {
                setTorchOn(true);
            }
            // LEWA END
            ...
            if (down) {
///LEWA BEGIN
                if(mVolumeUpKeyTriggered && releaseScreenOnProximitySensor()) {
                    return -1;
                }
///LEWA END
            ...
            if (down) {
                if (repeatCount == 0) {
                    ...
                    ///LEWA BEGIN
                } else if ((event.getFlags() & KeyEvent.FLAG_LONG_PRESS) != 0 && !keyguardOn) {
...
///LEWA BEGIN
//add by zhangxianjia for touchlight
    private void setTorchOn(boolean torchon) {
        synchronized(lock){
            Intent i;
            if(torchon && !getTorchState() || !torchon && getTorchState()){
                //Log.d("zhangxianjia", "set torchlight  torchon + " + String.valueOf(torchon));
                i = new Intent("com.lewa.flashlight.TOGGLE_FLASHLIGHT");
                mContext.sendBroadcast(i);
                i = new Intent("com.lewa.flashlight.lockscreen");
                i.putExtra("IS_ENABLE", torchon);
                mContext.sendBroadcast(i);
            }
        }
    }
        ...
        if (!isDefaultDisplay) {
            ...
            if ((fl & (FLAG_LAYOUT_IN_SCREEN | FLAG_FULLSCREEN | FLAG_LAYOUT_INSET_DECOR))
                ...
                if (attached != null) {
                    ...
                    /// M: [ALPS00411109] [ALPS00435427] The statu bar disappear and a blank area displayed. @{
                    if (mfindingTopFullscreenOpaque && !mHasSystemNavBar) {
                ...
                /// M: [ALPS00435427] The statu bar disappear and a blank area displayed.
                ...
                boolean nonFullScreenApplyStable =
                        ...
                        (fl & FLAG_FULLSCREEN) == 0;
                if (mfindingTopFullscreenOpaque && !nonFullScreenApplyStable && !mHasSystemNavBar) {
...
///LEWA END
    ...
    BroadcastReceiver mKeyDispatchReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (DEBUG_INPUT) {
                Log.v(TAG, "mKeyDispatchReceiver -- onReceive -- exist" + mKeyDispatcMode);
            }
        }
    };
...
///LEWA BEGIN
        releaseScreenOnProximitySensor();//Added by wangqiang at 2013-09-07
///LEWA END
    }
...
///LEWA BEGIN
        if (mSystemReady && mProximitySensor != null && !mDisableProximitor && getInadvertEnable()) {
            Log.i(TAG, "screenTruningOn  ............");
            mProximitySensor.aquire();//Added by wangqiang at 2013-09-07
        }
///LEWA END
    }
...
///LEWA BEGIN
            mProximitySensor = new LewaScreenOnProximityLock(mContext, mKeyguardMediator);//Added by wangqiang at 2013-09-07
///LEWA END
        }
        synchronized (mLock) {
            updateOrientationListenerLp();
...
///LEWA BEGIN
    ...
    static class Injector{
        static  MagnifierPopupWindow getMagnifier(PhoneWindowManager manager) {
            MagnifierPopupWindow magnifierpopupwindow;
            synchronized(manager) {
                if(manager.mMagnifier == null) {
                    int maxLayer = 1000 + 10000 *manager.windowTypeToLayerLw(TYPE_POINTER);
                    manager.mMagnifier = new MagnifierPopupWindow(manager.mContext, maxLayer);
                }
                magnifierpopupwindow = manager.mMagnifier;
            }
            return magnifierpopupwindow;
        }

        static void RegisteMagnifierReceiver(Context context,PhoneWindowManager manager) {
            IntentFilter magnifierFilter = new IntentFilter();
            magnifierFilter.addAction("android.intent.action.SHOW_MAGNIFIER");
            context.registerReceiver(manager.mShowMagnifierReceiver, magnifierFilter);
            manager.mMagnifierInputChannel = manager.mWindowManagerFuncs.monitorInput("Magnifier");
            manager.mMagnifierInputEventReceiver = manager.new MagnifierInputEventReceiver(
                    manager.mMagnifierInputChannel, Looper.myLooper());
        }
    }

    @LewaHook(LewaHook.LewaHookType.NEW_CLASS)
    private final class MagnifierInputEventReceiver extends InputEventReceiver {
        public MagnifierInputEventReceiver(InputChannel inputChannel,
                Looper looper) {
            super(inputChannel,looper);
        }
                ...
                try {
                    ...
                    synchronized (localObject2) {
                        ...
                        case 1:
                            mIsTouchDown = false;
                            if (mShowMagnifier) {
                                Injector.getMagnifier(PhoneWindowManager.this).hide();
                    ...
                    finishInputEvent(inputEvent, handled);
                }
        }
    }
///LEWA END
