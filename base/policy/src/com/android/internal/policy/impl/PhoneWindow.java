...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    private final class DecorView extends FrameLayout implements RootViewSurfaceTaker {
        ...
        protected void dispatchDraw(Canvas canvas) {
            ...
            // Woody Guo @ 2013/04/10:
            // Do not draw the corner drawables if the keyguard is currently locked
            // Resolves issues: 21713 22007
            if (getKeyguardManager().isKeyguardLocked()) {
                return;
            }

            if(mRoundedCorners == null) {
                mRoundedCorners = new RoundedCorners(getContext());
            }
            if(mFrameOffsets.left == 0 && mFrameOffsets.right == 0 && mFrameOffsets.bottom == 0) {
                mRoundedCorners.draw(canvas, 
                    mDrawingBounds.left, mDrawingBounds.top + mFrameOffsets.top, mDrawingBounds.right, mDrawingBounds.bottom);
            }
        }
        ///LEWA END
...
/// LEWA BEGIN
    //add by shenqi for bottom actionbar overlay
        if (a.getBoolean(com.android.internal.R.styleable.Window_windowActionBarBottomOverlay, false)) {
            requestFeature(FEATURE_ACTION_BAR_BOTTOM_OVERLAY);
        }
        //end by shenqi
/// LEWA END
        ...
        if ((features & ((1 << FEATURE_LEFT_ICON) | (1 << FEATURE_RIGHT_ICON))) != 0) {
            ...
            if (mIsFloating) {
                ...
                if ((features & (1 << FEATURE_ACTION_BAR_OVERLAY)) != 0) {
                    ///LEWA MODIFY BEGIN
                    layoutResource = Injector.getActionBarOverlayResourceId(getContext());
                    ///LEWA MODIFY END
                }
                /// LEWA BEGIN
                //add by lewa shenqi for button_bar_overlay
                else if ((features & (1 << FEATURE_ACTION_BAR_BOTTOM_OVERLAY)) != 0) {
                    layoutResource = Injector.getActionBarBottomOverlayResourceId(getContext());//com.android.internal.R.layout.screen_action_bar_bottom_overlay;
                } else {
                //end by shenqi
                /// LEWA END
                    ///LEWA MODIFY BEGIN
                    layoutResource = Injector.getActionBarResourceId(getContext());
                    ///LEWA MODIFY END
                }
            } else {
    ...
    static class Injector {
        static int getActionBarBottomOverlayResourceId(Context context) {
            if (LewaUiUtil.isV5Ui(context)) {
                return com.lewa.internal.R.layout.screen_action_bar_bottom_overlay;
            } else {
                return com.android.internal.R.layout.screen_action_bar_bottom_overlay;
            }
        }

        static int getActionBarOverlayResourceId(Context context) {
            if (LewaUiUtil.isV5Ui(context)) {
                return com.lewa.internal.R.layout.screen_action_bar_overlay;
            } else {
                return com.android.internal.R.layout.screen_action_bar_overlay;
            }
        }

        static int getActionBarResourceId(Context context) {
            if (LewaUiUtil.isV5Ui(context)) {
                return com.lewa.internal.R.layout.screen_action_bar;
            } else {
                return com.android.internal.R.layout.screen_action_bar;
            }
        }
    }
    ///LEWA ADD END
