...
public class EmergencyButton extends Button {
    ...
    KeyguardUpdateMonitorCallback mInfoCallback = new KeyguardUpdateMonitorCallback() {
        ...
        public void onRefreshCarrierInfoGemini(CharSequence plmn, CharSequence spn, int simId) {
            ...
            if (PhoneConstants.GEMINI_SIM_1 == simId) {
                State simState = KeyguardUpdateMonitor.getInstance(mContext).getSimState(PhoneConstants.GEMINI_SIM_1);
                updateEmergencyCallButton(simState, phoneState);

            } else {
                State simState = KeyguardUpdateMonitor.getInstance(mContext).getSimState(PhoneConstants.GEMINI_SIM_2);
                updateEmergencyCallButton(simState, phoneState);
            }
        }
        /// M @}
    ...
    private void updateEmergencyCallButton(State simState, int phoneState) {
        ...
        /// M:CTA new feature
///LEWA BEGIN
        boolean eccShouldShow = true;//eccButtonShouldShow();
///LEWA END
    ...
    private boolean eccButtonShouldShow(){
        ...
        if (KeyguardUtils.isGemini()) {
            CharSequence mPlmn = KeyguardUpdateMonitor.getInstance(this.getContext()).getTelephonyPlmn(PhoneConstants.GEMINI_SIM_1);
            CharSequence mSpn = KeyguardUpdateMonitor.getInstance(this.getContext()).getTelephonySpn(PhoneConstants.GEMINI_SIM_1);
            ...
            CharSequence mPlmnGemini = KeyguardUpdateMonitor.getInstance(this.getContext()).getTelephonyPlmn(PhoneConstants.GEMINI_SIM_2);
            CharSequence mSpnGemini = KeyguardUpdateMonitor.getInstance(this.getContext()).getTelephonySpn(PhoneConstants.GEMINI_SIM_2);
            ...
            CharSequence mPlmn = KeyguardUpdateMonitor.getInstance(this.getContext()).getTelephonyPlmn();
            CharSequence mSpn = KeyguardUpdateMonitor.getInstance(this.getContext()).getTelephonySpn();
            boolean mPlmnIsNotDefault = mPlmn!=null && !mPlmn.toString().equals(plmnDefaultStr);
            boolean mSpnIsNotDefault = mSpn!=null && !mSpn.toString().equals(plmnDefaultStr);
