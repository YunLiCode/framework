...
import android.annotation.LewaHook.LewaHookType;
///LEWA END
...
///LEWA BEGIN
    private boolean nLewaLockscreen = false;
///LEWA END
     ...
     * lazily.
     */
    @LewaHook(LewaHookType.CHANGE_CODE)
...
/// LEWA END
    ...
    class ViewManagerHost extends FrameLayout {
        ...
        protected void onConfigurationChanged(Configuration newConfig) {
            ...
            if (mKeyguardHost.getVisibility() == View.VISIBLE && mCreateOrientation != newConfig.orientation) {
...
///LEWA BEGIN
        boolean lewaChange = (0 != Settings.System.getInt(
                mContext.getContentResolver(), Settings.System.LOCKSCREEN_CHANGED, 0));
        boolean lewaLockscreen = KeyguardUtils.isLewaLockscreen(mContext);

        if (force || mKeyguardView == null || lewaLockscreen != nLewaLockscreen || (lewaChange && nLewaLockscreen)) {
            nLewaLockscreen = lewaLockscreen;
...
///LEWA BEGIN
        if(KeyguardUtils.isLewaLockscreen(mContext)){
            resId = R.id.lewa_keyguard_host_view;
            layoutId = R.layout.lewa_keyguard_host_view;
        }
///LEWA END
    ...
    public synchronized void reset(Bundle options) {
        ...
        if (options != null) {
            if (options.getBoolean(KeyguardViewMediator.RESET_FOR_DM_LOCK)
                ...
                || options.getBoolean(KeyguardViewMediator.RESET_LEWALOCKSCREEN)) {
                forceReCreate = true;
            }
        }
    ...
    public void reLayoutScreen(boolean dmLock) {
        if (mWindowLayoutParams != null) {
            KeyguardUtils.xlogD(TAG, "reLayoutScreen, dmLock=" + dmLock);
            if (dmLock) {
...
///LEWA BEGIN
    public boolean isLewaLockscreen(){
        return nLewaLockscreen;
    }
///LEWA END
...
///LEWA BEGIN
    @LewaHook(LewaHookType.NEW_METHOD)
    KeyguardHostView getKeyguardView() {
    ...
    @LewaHook(LewaHookType.NEW_CLASS)
    static class Injector {
        static void updateDisplayDesktopFlag(KeyguardViewManager manager) {
            KeyguardHostView keyguardView = manager.getKeyguardView();
            WindowManager.LayoutParams params = manager.getWindowLayoutParams();
            if (keyguardView != null) {
                boolean displayDesktop = getProperty(keyguardView, "displayDesktop", false);
                boolean showSysWallpaper = getProperty(keyguardView, "showSysWallpaper", false);
                if (displayDesktop && !manager.isSecure()) {
                    params.flags &= ~WindowManager.LayoutParams.FLAG_SHOW_WALLPAPER;
                    params.privateFlags |= WindowManager.LayoutParams.PRIVATE_FLAG_SHOW_DESKTOP;
                } else {
                    if (showSysWallpaper)
                        params.flags |= WindowManager.LayoutParams.FLAG_SHOW_WALLPAPER;
                    else
        ...
        private static boolean getProperty(KeyguardHostView keyguardView, String name, boolean def) {
            try {
                KeyguardSecurityView v = keyguardView.getCurrentKeyguardSecurityView();
                return Boolean.parseBoolean((String) v.getClass()
                        .getMethod("getProperty", String.class).invoke(v, name));
            } catch (Exception e) {
                if (DEBUG)
