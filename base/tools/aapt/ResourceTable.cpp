...
///LEWA BEGIN
    status_t createIfNeeded(ResourceTable* outTable, bool ignoreRedefinedError=false)
///LEWA END
    {
        if (added || hasErrors) {
            return NO_ERROR;
...
///LEWA BEGIN
            // Woody Guo @ 2012/12/04: Do not try to re-create the attribute if the property "added" exists
            // This is to eliminate the error "Attribute "findOnPageNextDrawable" has already been defined"
            // if an attribute defined in overlay resources is already defined in original attrs.xml
            if (ignoreRedefinedError) {
                added = false;
                return NO_ERROR;
            }
///LEWA END
...
///LEWA BEGIN
    ...
    bool existed = false;
    if (typeIdx >= 0) {
        ignoreError = (block.indexOfAttribute(NULL, "added") >= 0);
///LEWA END
...
///LEWA BEGIN
        attr.createIfNeeded(outTable, ignoreError);
        if (!attr.added && ignoreError) {
            existed = true;
        }
///LEWA END
    } else if (!inStyleable) {
...
///LEWA BEGIN
        if (!existed) attr.createIfNeeded(outTable);
        if (!existed && !attr.hasErrors) {
///LEWA END
...
///LEWA BEGIN
            if (!existed && !attr.hasErrors && !outTable->hasBagOrEntry(itemIdent, &id16, &myPackage)) {
///LEWA END
...
///LEWA BEGIN
    if (!existed && !attr.hasErrors && enumOrFlagsComment.size() > 0) {
///LEWA END
        enumOrFlagsComment.append(String16("\n</table>"));
    ...
    while ((code=block.next()) != ResXMLTree::END_DOCUMENT && code != ResXMLTree::BAD_DOCUMENT) {
        if (code == ResXMLTree::START_TAG) {
            ...
            if (strcmp16(block.getElementName(&len), skip16.string()) == 0) {
                ...
                curIsBagReplaceOnOverwrite = true;
            ///LEWA BEGIN
            // Handle java-symbol introduced in Jelly Bean
            } else if (strcmp16(block.getElementName(&len), java_symbol16.string()) == 0) {
                while ((code=block.next()) != ResXMLTree::END_DOCUMENT
                        && code != ResXMLTree::BAD_DOCUMENT) {
                    if (code == ResXMLTree::END_TAG) {
