...
///LEWA BEGIN
        try {
            ...
            a.recycle();
            if (mBackgroundLeft != null && mBackgroundRight != null) {
                mHasRoundTab = true;
            }
        } catch (Exception e) {
        }
...
///LEWA BEGIN
    private void updateTabBackground() {
        if (mHasRoundTab) {
            ...
            if (count > 0) {
                if (count == 1) {
                    mTabLayout.getChildAt(0).setBackground(mBackgroundMiddle);
                } else {
                    for (int i = 0; i < count; i++) {
                        View child = mTabLayout.getChildAt(i);
                        if (i == 0) {
                            child.setBackground(mBackgroundLeft);
                        } else if (i == count - 1) {
                            child.setBackground(mBackgroundRight);
                        } else {
                            child.setBackground(mBackgroundMiddle.getConstantState().newDrawable());
                        }
                    }
                }
            }
