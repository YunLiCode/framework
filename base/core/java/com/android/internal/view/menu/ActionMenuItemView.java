...
public class ActionMenuItemView extends TextView
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        ///LEWA BEGIN
        // Set paddingLeft and drawable padding so it centers the icon/title of this action item
        this.setGravity(Gravity.CENTER);
        if (mAllowTextWithIcon && textVisible && null != mIcon
                && mItemData.requiresActionButton() && mItemData.showsTextAsAction()) {
            this.setGravity(Gravity.CENTER_VERTICAL);
            final int w = getMeasuredWidth();
            final int dw = mIcon.getIntrinsicWidth();
            android.text.TextPaint mTextPaint = getPaint();
            final int tw = (int) android.text.Layout.getDesiredWidth(mTitle, mTextPaint);
            final int spacing = (int) android.util.TypedValue.applyDimension(
                    android.util.TypedValue.COMPLEX_UNIT_DIP, 8
                    , getContext().getResources().getDisplayMetrics());
            final int paddingLeft = (w - dw - tw - spacing) / 2 - getPaddingRight();
            super.setPadding(paddingLeft, getPaddingTop(), getPaddingRight(), getPaddingBottom());
            setCompoundDrawablePadding(spacing);
            /*
             * android.util.Log.d("ActionMenuItemView", "Left padding: " + paddingLeft
             *         + ", drawable padding: " + getCompoundPaddingLeft()
             *         + ":" + (- paddingLeft + (w / 2 - paddingLeft - dw))
             *         + ", text width: " + tw + ", drawable width: " + dw
             *         + ", width: " + w + ", title: " + mTitle
    ...
    static class Injector {
        static int measureMidy(ActionMenuItemView itemView, int y, int height) {
            if (LewaUiUtil.isV5Ui(itemView.getContext())) {
                return y + height;
            } else {
                return y + height / 2;
            }
