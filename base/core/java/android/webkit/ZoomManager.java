...
class ZoomManager {
    ...
    public void onSizeChanged(int w, int h, int ow, int oh) {
        ...
        /// M: fix CR ALPS00312730, the NEW_PICTURE_MSG_ID cannot be done first. @{
        ...
        if (bBaiduURL && mWebView.mPrivateHandler.hasMessages(WebViewClassic.NEW_PICTURE_MSG_ID)) {
            mWebView.mPrivateHandler.removeMessages(WebViewClassic.NEW_PICTURE_MSG_ID);
        }
        /// @}
    ...
    public boolean onNewPicture(WebViewCore.DrawData drawData) {
        ...
        if (!mMinZoomScaleFixed || settings.getUseWideViewPort()) {
            /// M: fix ALPS00323727, do not modify min and max zoom scale if the two value are the same. @{
            if (mMinZoomScale == mMaxZoomScale && mMinZoomScaleFixed) {
                ...
                mMaxZoomScale = newZoomScale;//newZoomOverviewScale;
                mMinZoomScale = newZoomScale;
                setZoomScale(mMinZoomScale, false);
            } else {
