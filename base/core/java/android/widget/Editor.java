...
import lewa.os.LewaCursorController;
import lewa.os.MagnifierController;
...
//    SelectionModifierCursorController mSelectionModifierCursorController;
    LewaCursorController mInsertionPointCursorController;
    LewaCursorController mSelectionModifierCursorController;
    ...
    private boolean touchPositionIsInSelection() {
        ...
        //LEWA BEGIN
        LewaCursorController selectionController = getSelectionController();
        //LEWA END
    ...
    @LewaHook(LewaHook.LewaHookType.CHANGE_CODE)
    public static interface TextViewPositionListener {
    ...
    public boolean isPositionVisibleWrap(int positionX, int positionY) {
        return isPositionVisible(positionX, positionY);
    }
   @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public void addPositionListenerSubscriber(TextViewPositionListener listener,
            boolean canMove) {
        getPositionListener().addSubscriber(listener, canMove);
    }
   @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    boolean isOffsetVisibleWrap(int offset) {
        return isOffsetVisible(offset);
    }
    //LEWA ADD END
    ...
    public boolean performLongClick(boolean handled) {
        return Injector.performLongClick(Editor.this, handled);
    }
    @LewaHook(LewaHook.LewaHookType.CHANGE_CODE)
...
//        }
    ...
    @LewaHook(LewaHook.LewaHookType.CHANGE_CODE)
    public LewaCursorController getInsertionController() {
        if (!mInsertionControllerEnabled) {
            return null;
        }
        if (mInsertionPointCursorController == null) {
            mInsertionPointCursorController = LewaCursorController.create(this,
                mContext, 1);
            final ViewTreeObserver observer = mTextView.getViewTreeObserver();
            observer.addOnTouchModeChangeListener(mInsertionPointCursorController);
        }
    ...
    @LewaHook(LewaHook.LewaHookType.CHANGE_CODE)
    public  LewaCursorController getSelectionController() {
        if (!mSelectionControllerEnabled) {
            return null;
        }

        if (mSelectionModifierCursorController == null) {
            mSelectionModifierCursorController = LewaCursorController.create(
                this, mContext, 2);
            final ViewTreeObserver observer = mTextView.getViewTreeObserver();
            observer.addOnTouchModeChangeListener(mSelectionModifierCursorController);
        }
...
///LEWA END
    ...
    public MagnifierController getMagnifierController() {
        boolean _tmp = false;
        boolean flag = hasMagnifierController();
    ...
    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public ArrayList<Boolean> getFloatPanelShowHides(int[] buttons) {
        ArrayList showHides = new ArrayList();
        Boolean isTextEditable = Boolean.valueOf(mTextView.isTextEditable());
        for (int button : buttons) {
            showHides.add(Boolean.valueOf(getButtonShowHides(
                    isTextEditable.booleanValue(), button)));
        }
        return showHides;
    }
    ...
    boolean getButtonShowHides(boolean isTextEditable, int id) {
        if(!isTextEditable)
            return false;
        switch(id){
        case com.lewa.internal.R.id.buttonSelect:
            return mTextView.canSelectTextWrap();
        case com.lewa.internal.R.id.buttonSelectAll:
            return mTextView.canSelectTextWrap();
        case com.lewa.internal.R.id.buttonPaste:
            return mTextView.canPaste();
        case com.lewa.internal.R.id.buttonPasteList:
            ClipboardManager clip = (ClipboardManager)mContext.getSystemService("clipboard");
            List<String> histroy=clip.getData(1);
            if(histroy!=null&&histroy.size()>0){
                    return true;
            }else{
                return false;
            }
        case com.lewa.internal.R.id.buttonCopy:
            return mTextView.canCopy();
        case com.lewa.internal.R.id.buttonCut:
            return mTextView.canCut();
        }
        return true;
    }

   @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public Drawable getSelectHandleCenterRes() {
        if (mSelectHandleCenter == null)
    ...
    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public Drawable getSelectHandleLeftRes() {
        if (mSelectHandleLeft == null)
    ...
    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public Drawable getSelectHandleRightRes() {
        if (mSelectHandleRight == null)
    ...
    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public boolean selectAllWrap() {
        return mTextView.selectAllText();
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public long getCharRangeWrap(int offset) {
        return getCharRange(offset);
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public void onTapUpEvent() {
        if (!mDiscardNextActionUp)
            Injector.stopTextSelectionMode(Editor.this);
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public void startTextSelectionModeIfDouleTap(long previousTapUpTime, float x,
            float y, float previousX, float previousY) {
        ...
        if ((duration <= ViewConfiguration.getDoubleTapTimeout())
            ...
            if ((distanceSquared < touchSlop * touchSlop)
                    && (Injector.startTextSelectionMode(Editor.this)))
                mDiscardNextActionUp = true;
        }
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public void handleFloatPanelClick(View v, LewaCursorController cc) {
        int min;
        int max = mTextView.length();

        boolean bool = mTextView.isFocused();
        int i = 0;
        if (bool) {
            int selStart = mTextView.getSelectionStart();
            int selEnd = mTextView.getSelectionEnd();
            i = Math.max(0, Math.min(selStart, selEnd)); 
        ...
        switch (v.getId()) {
            case com.lewa.internal.R.id.buttonCut:
                LewaCursorController.SelectionModifierCursorController localSelectionModifierCursorController2 = (LewaCursorController.SelectionModifierCursorController)cc; 
                localSelectionModifierCursorController2.addClipData(clip, data, mTextView.getTransformedText(i, max)); 
                Injector.stopTextSelectionMode(Editor.this);
                mTextView.deleteText_internal(i, max);
//                Toast.makeText(mContext, mContext.getResources().getString(17040299), 0).show();
                return;
            case com.lewa.internal.R.id.buttonCopy:
                LewaCursorController.SelectionModifierCursorController localSelectionModifierCursorController1 = (LewaCursorController.SelectionModifierCursorController)cc; 
                localSelectionModifierCursorController1.addClipData(clip, data, mTextView.getTransformedText(i, max)); 
                Injector.stopTextSelectionMode(Editor.this);
//                Toast.makeText(mContext, mContext.getResources().getString(17040299), 0).show();
                return;
            case com.lewa.internal.R.id.buttonPasteList:
                CharSequence paste;
                if (data != null) {
                    l = new PastePanelOnClickListener(cc, i, max);
                    LewaCursorController.InsertionPointCursorController localInsertionPointCursorController = (LewaCursorController.InsertionPointCursorController)cc;
                    localInsertionPointCursorController.setupClipBoardPanel(data, l);
                    return;
                }
                break;
            case com.lewa.internal.R.id.buttonPaste:
                int j = data.getItemCount();
                CharSequence localCharSequence1 = null;
                if (j > 0)
                    localCharSequence1 = data.getItemAt(0).coerceToText(mContext);
                if ((localCharSequence1 != null) && (localCharSequence1.length() > 0)) {
                    Selection.setSelection((Spannable)mTextView.getText(), max);
                    mTextView.replaceText_internal(i, max, localCharSequence1);
                    Injector.stopTextSelectionMode(Editor.this);
                    return;
                }
                break;
            case com.lewa.internal.R.id.buttonSelectAll:
                if (mSelectionControllerEnabled) {
                    ...
                    if (Injector.startTextSelectionMode(Editor.this)) {
                        getSelectionController().show();
                        return;
                    }
                }
                break;
            case com.lewa.internal.R.id.buttonSelect:
                if ((mSelectionControllerEnabled) && (Injector.startTextSelectionMode(Editor.this))) {
                    getSelectionController().show();
                    return;
                }
                break;
    ...
    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public void removePositionListenerSubscriberWrap(TextViewPositionListener listener) {
        removePositionListenerSubscriber(listener);
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    void removePositionListenerSubscriber(TextViewPositionListener listener) {
        getPositionListener().removeSubscriber(listener);
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public void setTextSelectionWrap(int start, int end) {
        Selection.setSelection((Spannable) mTextView.getText(), start, end);
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public void setTextSelectionWrap(int offset) {
        setTextSelection(offset);
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public int getSelectHandleWindowStyle() {
    ...
    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    void setTextSelection(int offset) {
        Spannable text = (Spannable) mTextView.getText();
        int start = offset;
        int stop = offset;
        int candStart = EditableInputConnection.getComposingSpanStart(text);
        int candEnd = EditableInputConnection.getComposingSpanEnd(text);
        if (candStart >= 0 && candEnd >= 0 && candStart != candEnd) {
            stop = Math.max(candStart, candEnd);
            start = stop;
        }
        Selection.setSelection((Spannable) mTextView.getText(), start, stop);
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public boolean hasMagnifierController() {
        return ( (mInsertionControllerEnabled)&& (mSelectionControllerEnabled));
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public boolean hasInsertionControllerWrap() {
        return hasInsertionController();
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public boolean hasSelectionControllerWrap() {
        return hasSelectionController() ;
    }
    ...
    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public long getPositionListenerPostion() {
        int x = getPositionListener().getPositionX();
        int y = getPositionListener().getPositionY();
        return TextUtils.packRangeInLong(x, y);
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public boolean needInsertPanel() {
        return (!mIsInTextSelectionMode)
                && ((canSelectText()) || (mTextView.canPaste()));
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public void hideInsertionPointCursorControllerWrap() {
        hideInsertionPointCursorController();
    }

    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public void makeBlinkWrap(){
        makeBlink();
    }
    ...
    static class Injector{
        static boolean performLongClick(Editor editor,boolean handled){
            if (true) {
                boolean _tmp = false;
                boolean flag =  editor.hasMagnifierController();
                boolean flag1 = false;
                if (flag) {
                    Log.i("MiuiMagnifierController",
                            "performLongClick to show MagnifierController");
                    stopTextSelectionMode(editor);
                    editor.getSelectionController().hide();
                    editor.getInsertionController().hide();
                    editor.getMagnifierController().show();
                    editor.getInsertionController().show();
                    editor.mDiscardNextActionUp = true;
                    flag1 = true;
                }
                boolean flag2 = false;
                if(!flag1)
                if(editor.mIsInTextSelectionMode && editor.touchPositionIsInSelection()) {
                    stopTextSelectionMode(editor);
                    flag1 = true;
                } else {
                    stopTextSelectionMode(editor);
                    flag1 = startTextSelectionMode(editor);
                    flag2 = flag1;
                }
                if (flag2)
                    editor.mTextView.performHapticFeedback(0);
                if (flag1)
                    editor.mDiscardNextActionUp = true;
                return flag1;
            } else{
                // Long press in empty space moves cursor and shows the Paste affordance if available.
                if (!handled && !editor.isPositionOnText(editor.mLastDownPositionX, editor.mLastDownPositionY) &&
                        editor.mInsertionControllerEnabled) {
                    final int offset =editor.mTextView.getOffsetForPosition(editor.mLastDownPositionX,
                            editor.mLastDownPositionY);
                    editor.stopSelectionActionMode();
                    Selection.setSelection((Spannable) editor.mTextView.getText(), offset);
//                    editor.getInsertionController().showWithActionPopup();
                    handled = true;
                }

                if (!handled && editor.mSelectionActionMode != null) {
                    if (editor.touchPositionIsInSelection()) {
                        // Start a drag
                        final int start = editor.mTextView.getSelectionStart();
                        final int end =editor.mTextView.getSelectionEnd();
                        CharSequence selectedText = editor.mTextView.getTransformedText(start, end);
                        ClipData data = ClipData.newPlainText(null, selectedText);
                        DragLocalState localState = new DragLocalState(editor.mTextView, start, end);
                        editor.mTextView.startDrag(data, editor.getTextThumbnailBuilder(selectedText), localState, 0);
                        editor.stopSelectionActionMode();
                    } else {
                        editor.getSelectionController().hide();
                        editor.selectCurrentWord();
                        editor.getSelectionController().show();
                    }
                    handled = true;
                }
                ...
                if (!handled) {
                    handled = editor.startSelectionActionMode();
                }
        ...
        static void stopTextSelectionMode(Editor editor) {
            if (editor.mIsInTextSelectionMode) {
                Selection.setSelection((Spannable) (editor.mTextView).getText(),
                        editor.mTextView.getSelectionEnd());
                if (editor.mSelectionModifierCursorController != null)
                    editor.mSelectionModifierCursorController.hide();
                editor.mIsInTextSelectionMode = false;
            }
        }

        static boolean startTextSelectionMode(Editor editor) {
            if (!editor.mIsInTextSelectionMode)
                if (!editor.hasSelectionController()) {
                    Log.w("Editor",
                            "TextView has no selection controller. Selection mode cancelled.");
                } else if ((editor.canSelectText()) && (editor.mTextView.requestFocus())) {
                    if ((!editor.mTextView.hasSelection())
                            && (!editor.getSelectionController().selectCurrentRange())) {
                        Log.i("Editor", "can not selection current range");
                        return false;
                    }

                    editor.getSelectionController().show();
                    editor.mIsInTextSelectionMode = true;
                    return true;
                }
    ...
    @LewaHook(LewaHook.LewaHookType.NEW_CLASS)
    private class PastePanelOnClickListener implements View.OnClickListener {
        private LewaCursorController mController;
        private int mMax;
        private int mMin;

        PastePanelOnClickListener(LewaCursorController paramInt1,
                int paramInt2, int max) {
            mController = paramInt1;
            mMin = paramInt2;
            mMax = max;
        }
        ...
        public void onClick(View v) {
            ...
            Injector.stopTextSelectionMode(Editor.this);
            ((LewaCursorController.InsertionPointCursorController) mController)
                    .onClipBoardPancelClick();
        }
    }
    //LEWA ADD END
}
