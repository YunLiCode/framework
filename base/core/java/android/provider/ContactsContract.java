...
///LEWA BEGIN
    public static final class StickerDisplayPhoto implements BaseColumns, DataColumnsWithJoins {
            /**
             * no public constructor since this is a utility class
             */
            private StickerDisplayPhoto() {}

            /**
             * The directory twig for this sub-table
             */
            public static final String CONTENT_DIRECTORY = "sticker_display_photo";

            /**
             * Full-size photo file ID of the raw contact.
             * See {@link ContactsContract.StickerDisplayPhoto}.
             * <p>
             * Type: NUMBER
             */
            public static final String STICKER_PHOTO_FILE_ID = DATA14;
    ...
    public static InputStream openContactStickerPhotoInputStream(ContentResolver cr, Uri contactUri) {
            final Uri stickerdisplayPhotoUri = Uri.withAppendedPath(contactUri,
                    Contacts.StickerDisplayPhoto.CONTENT_DIRECTORY);
            InputStream inputStream;
            try {
                AssetFileDescriptor fd = cr.openAssetFileDescriptor(stickerdisplayPhotoUri, "r");
                return fd.createInputStream();
            } catch (IOException e) {
...
///LEWA BEGIN
         ...
         * <p>
         * A sub-directory of a single raw contact that represents its primary
         * sticker display photo.  To access this directory append
         * {@link RawContacts.StickerDisplayPhoto#CONTENT_DIRECTORY} to the raw contact URI.
         * The resulting URI represents an image file, and should be interacted with
         * using ContentResolver.openAssetFileDescriptor.
         * <p>
         * <p>
         * Note that this sub-directory also supports opening the photo as an asset file
         * in write mode.  Callers can create or replace the primary photo associated
         * with this raw contact by opening the asset file and writing the full-size sticker
         * photo contents into it.  When the file is closed, the image will be parsed,
         * sized down if necessary for the full-size display photo and thumbnail
         * dimensions, and stored.
         * </p>
         ...
         * Usage example:
         * <pre>
         * public void StickerDisplayPhoto(long rawContactId, byte[] photo) {
         *     Uri rawContactPhotoUri = Uri.withAppendedPath(
         *             ContentUris.withAppendedId(RawContacts.CONTENT_URI, rawContactId),
         *             RawContacts.StickerDisplayPhoto.CONTENT_DIRECTORY);
         *     try {
         *         AssetFileDescriptor fd =
         *             getContentResolver().openAssetFileDescriptor(rawContactPhotoUri, "rw");
         *         OutputStream os = fd.createOutputStream();
         ...
         * </p>
         */
         public static final class StickerDisplayPhoto {
            /**
             * No public constructor since this is a utility class
             */
            private StickerDisplayPhoto() {
...
///LEWA BEGIN
	  /**
         * <p>
         * A data kind representing a sticker photo for the contact.
         * </p>
         * <p>
         ...
         * </table>
         */
        public static final class StickerPhoto implements DataColumnsWithJoins {
            /**
             * This utility class cannot be instantiated
             */
            private StickerPhoto() {}

            /** MIME type used when storing this in data table. */
            public static final String CONTENT_ITEM_TYPE = "vnd.android.cursor.item/sticker_display_photo";
            ...
            public static final String PHOTO_Y = DATA13;            //LeWa jxli
        }
///LEWA END
        /**
         * <p>
...
///LEWA BEGIN
      /**
     * Helper class for accessing full-size photos by sticker photo file ID.
     * <p>
     * Usage example:
     * <dl>
     * <dt>Retrieving a full-size photo by sticker photo file ID (see
     * {@link ContactsContract.ContactsColumns#STICKER_PHOTO_FILE_ID})
     * </dt>
     * <dd>
     * <pre>
     * public InputStream openStickerDisplayPhoto(long stickerphotoFileId) {
     *     Uri stickerdisplayPhotoUri = ContentUris.withAppendedId(StickerDisplayPhoto.CONTENT_URI, photoKey);
     *     try {
     *         AssetFileDescriptor fd = getContentResolver().openAssetFileDescriptor(
     *             stickerdisplayPhotoUri, "r");
     *         return fd.createInputStream();
     *     } catch (IOException e) {
     *         return null;
     *     }
    ...
    public static final class StickerDisplayPhoto {
         ...
         * given a key.
         */
        public static final Uri CONTENT_URI = Uri.withAppendedPath(AUTHORITY_URI, "sticker_display_photo");

        /**
         * This URI allows the caller to query for the maximum dimensions of a sticker display photo.
         * Requests to this URI can be performed on the UI thread because
         * they are always unblocking.
         */
        public static final Uri CONTENT_MAX_DIMENSIONS_URI =
                Uri.withAppendedPath(AUTHORITY_URI, "sticker_photo_dimensions");

        /**
         * Queries to {@link ContactsContract.StickerDisplayPhoto#CONTENT_MAX_DIMENSIONS_URI} will
         * contain this column, populated with the maximum height and width (in pixels)
         * that will be stored for a sticker display photo.  Larger photos will be down-sized to
         * fit within a square of this many pixels.
         */
        public static final String DISPLAY_MAX_DIM = "sticker_display_max_dim";
    }
///LEWA END
    /**
...
///LEWA BEGIN
        public static final String CALL_LOG_COUNT = "call_log_count";
///LEWA END
    }
...
///LEWA BEGIN
    protected interface ViewDialerSearchColumns {
    	...
    	public static final String CALL_LOG_COUNT = "vds_call_log_count";
    	...
    	public static final String CONTACT_TIMES_CONTACTED = "vds_times_contacted";
    	public static final String CONTACT_LAST_DATE_CONTACTED = "vds_last_date_contacted";
