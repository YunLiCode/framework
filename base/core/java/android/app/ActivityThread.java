...
///LEWA BEGIN
        ...
        public boolean equals(Object obj) {
            ...
            if (mIsThemeable != peer.mIsThemeable) return false;
            return true;
        }
    }
///LEWA END
...
///LEWA BEGIN
    // Woody Guo lewa @ 2012/06/15: port T-Mobile theme engine from CM9
...
///LEWA BEGIN
        ...
        Configuration lewaconfig = getConfiguration();
        if (compInfo.isThemeable && lewaconfig != null) {
            if (lewaconfig.customTheme == null) {
                lewaconfig.customTheme = CustomTheme.getBootTheme(getSystemContext());
            }

            if (!TextUtils.isEmpty(lewaconfig.customTheme.getThemePackageName())) {
                if (localLOGV) Log.d("WOODY", "ActivityThread::getTopLevelResources - theme name:" + lewaconfig.customTheme.getThemePackageName());
                attachThemeAssets(assets, lewaconfig.customTheme);
            }
///LEWA END
        }
...
///LEWA BEGIN
    private void detachThemeAssets(AssetManager assets) {
        ...
        if (localLOGV) Log.d("WOODY", "ActivityThread::detachThemeAssets - theme name:" + themePackageName);
        int themeCookie = assets.getThemeCookie();
        if (!TextUtils.isEmpty(themePackageName) && themeCookie != 0) {
            if (localLOGV) Log.d("WOODY", "ActivityThread::detachThemeAssets - detaching theme " + themeCookie);
    ...
    /**
     * Woody Guo @ 2012/06/15: port T-Mobile theme engine from CM9
     *
    ...
    private boolean attachThemeAssets(AssetManager assets, CustomTheme theme) {
        ...
        if (localLOGV) Log.d(TAG, Thread.currentThread().getStackTrace()[2].getMethodName()
                + " - theme package name: " + theme.getThemePackageName());
        if (pi != null && pi.applicationInfo != null && pi.themeInfos != null) {
            String themeResDir = pi.applicationInfo.publicSourceDir;
            if (localLOGV) Log.d(TAG, Thread.currentThread().getStackTrace()[2].getMethodName()
                    + " - source dir: " + themeResDir);
            ...
            if (cookie != 0) {
                ...
                if (localLOGV) Log.d(TAG, Thread.currentThread().getStackTrace()[2].getMethodName()
                        ...
                        + " attached successfully; base package count: " + N);
                for (int i = 0; i < N; i++) {
                    String packageName = assets.getBasePackageName(i);
                    int packageId = assets.getBasePackageId(i);
                    if (localLOGV) Log.d(TAG
                            , "\t" + i + " base package name: " + packageName
                            + "; base package id: " + packageId);

                    /*
                     * For now, we only consider redirections coming from the
                     * frameworks or regular android packages. This excludes
                     * themes and other specialty APKs we are not aware of.
                     */
                    // if (packageId != 0x01 && packageId != 0x7f) {
                    if (packageId != 0x01 && packageId != 0x02
                            && packageId != 0x09 && packageId != 0x7f) {
                        continue;
                    }

                    if (localLOGV) Log.d(TAG, "\tgenerating redirection map...");
                    try {
...
///LEWA END
    ...
    private Activity performLaunchActivity(ActivityClientRecord r, Intent customIntent) {
        ...
        try {
            ...
            if (!mInstrumentation.onException(activity, e)) {
///LEWA BEGIN
                if (e instanceof InflateException || e instanceof Resources.NotFoundException) {
...
///LEWA BEGIN
    final /* boolean */ int applyConfigurationToResourcesLocked(Configuration config,
            CompatibilityInfo compat) {
...
///LEWA BEGIN
                ...
                if (themeChanged) {
                    if (localLOGV) Log.d("WOODY", "ActivityThread::applyConfigurationToResourcesLocked - theme changed");
                    AssetManager am = r.getAssets();
                    if (am.hasThemeSupport()) {
                        detachThemeAssets(am);
                        if (!TextUtils.isEmpty(config.customTheme.getThemePackageName())) {
                            if (localLOGV) Log.d("WOODY", "ActivityThread::applyConfigurationToResourcesLocked - theme name:" + config.customTheme.getThemePackageName());
        ...
        freeTextLayoutCachesIfNeeded(changes);
        return changes;//!= 0;
///LEWA END
    }
...
///LEWA BEGIN
                ...
                performConfigurationChanged(/* callbacks.get(i), */cb, config);
///LEWA END
            }
        }
    }
...
//            if (hasLocaleConfigChange) {
            boolean hasFontConfigChange = ((configDiff & ActivityInfo.CONFIG_FONT) != 0);
