...
public class DownloadManager {
     ...
     * The download manager will send notifications to a component in this package when the download completes.
     */
    public static final String COLUMN_NOTIFICATION_PACKAGE  = Downloads.Impl.COLUMN_NOTIFICATION_PACKAGE;
    
    /**
     * The content:// URI to access downloads owned by the caller's UID. same to Downloads.Impl.CONTENT_URI
     */
    public static final Uri CONTENT_URI  = Downloads.Impl.CONTENT_URI;
    
    /** thread pool to delete file **/
    public static ExecutorService THREAD_POOL_DELETE_FILE = Executors.newCachedThreadPool();
    
    /** extra key of user extra send to app when click item in download manager ui **/
    public static final String EXTRA_DOWNLOAD_USER_EXTRA = "extra_download_user_extra";
    ///LEWA END
    ...
    ///LEWA BEGIN
    /**
     * resume download, added by gxwu@lewatek.com 2013/03/01
     * 
    ...
    /**
     * pause download, added by gxwu@lewatek.com 2013/03/01
     * 
    ...
    /**
     * added by gxwu@lewatek.com 2013/03/01, same to {@link #markRowDeleted(long...)}<br/>
     * Cancel downloads and remove them from the download manager.  Each download will be stopped if
     * it was running, and it will no longer be accessible through the download manager.
     * If there is a downloaded file, it is not deleted.
     *
    ...
    /**
     * get status by id, added by gxwu@lewatek.com 2013/03/01
     * 
     * @param id the id of the download
     * @return the status of the download, in {@link #STATUS_*}
     */
    public int getStatusById(long id) {
        return translateStatus(getStatusInContentProviderById(id));
    }
    
    /**
     * always return false since 2013/03/26<br/>
     * get the value of download only allowed by wifi, it will be setted by com.android.providers.downloads.ui
     * 
     * @return return the value setted by com.android.providers.downloads.ui, if not exist or exception return false default
     */
    @Deprecated
    public boolean getDownloadOnlyWifi(Context context) {
        return false;
    }

    /**
     * always return true since 2013/03/26<br/>
     * get the value of whether remind when download by 2g/3g, it will be setted by com.android.providers.downloads.ui
     * 
     * @return return the value setted by com.android.providers.downloads.ui, if not exist or exception return true default
     */
    @Deprecated
    public boolean getDownload2GRemind(Context context) {
        return true;
    }
    
    /**
     * get download status in content provider<br/>
     * it equal to {@link #getStatusById(long)} if pass it to {@link #translateStatus(int))} as a para<br/>
     * added by gxwu@lewatek.com 2013/03/01
     * 
     * @param id
     * @return
     */
    public int getStatusInContentProviderById(long id) {
        int status = -1;
        Cursor cursor = null;
        try {
            cursor = mResolver.query(mBaseUri, new String[]{COLUMN_STATUS}, 
                                     new StringBuilder().append(Downloads.Impl._ID).append(" = ? ").toString(), 
    ...
    /**
     * get downloaded byte and total byte, the result[0] represents downloaded bytes, the result[1] represents total bytes<br/>
     * added by gxwu@lewatek.com 2013/03/01
     * 
     * @param id
     * @return
     */
    public int[] getDownloadBytes(long id) {
        int[] currentByte = new int[]{0, 0};
        Cursor cursor = null;
        try {
            cursor = mResolver.query(mBaseUri, new String[]{Downloads.Impl.COLUMN_CURRENT_BYTES, Downloads.Impl.COLUMN_TOTAL_BYTES}, 
                                     new StringBuilder().append(Downloads.Impl._ID).append(" = ? ").toString(), new String[]{Long.toString(id)}, null);
            if (cursor != null && cursor.moveToFirst()) {
                currentByte[0] = cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.Impl.COLUMN_CURRENT_BYTES));
                currentByte[1] = cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.Impl.COLUMN_TOTAL_BYTES));
            }
        } finally {
            if (cursor != null) {
                cursor.close();
    ...
    /**
     * get pause reason, added by gxwu@lewatek.com 2013/03/01
     * 
     * @param status the return value of {@link #getStatusInContentProviderById(long)}
     * @return {@link #PAUSED_*}
     */
    public static long getPausedReason(int status) {
        switch (status) {
    ...
    /**
     * get error code, added by gxwu@lewatek.com 2013/03/01
     * 
     * @param status the return value of {@link #getStatusInContentProviderById(long)}
     * @return {@link #ERROR_*}
     */
    public static long getErrorCode(int status) {
    ...
    /**
     * tranform status from STATUS_* in Downloads.Impl to STATUS_* in DownloadManager<br/>
     * added by gxwu@lewatek.com 2013/03/01
     * 
     * @param status the return value of {@link #getStatusInContentProviderById(long)}
     * @return {@link #STATUS_*}
     ...
     * @return the number of downloads actually removed
     */
    public int removeAndDeleteFile(long... ids) {
        final List<String> filePathList = getDownloadFilePath(ids);
        int result = markRowDeleted(ids);
        if (filePathList != null && filePathList.size() > 0) {
            THREAD_POOL_DELETE_FILE.execute(new Runnable() {
    ...
    private void deleteFileIfExists(String path) {
        try {
            ...
            Log.v(XLOGTAG, "file: '" + path + "' couldn't be deleted", e);
        }
    }
     ...
     * @return download file path list
     */
    public List<String> getDownloadFilePath(long... ids) {
        ...
        try {
            ...
            for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
                filePathList.add(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_LOCAL_FILENAME)));
            }
            return filePathList;
        } finally {
            if (cursor != null) {
