...
public class ClipboardManager extends android.text.ClipboardManager {
    ...
    public void setPrimaryClip(ClipData clip) {
        try {
            ///LEWA BEGIN
            Injector.addData(clip, mContext);
            ///LEWA END
...
//  @LewaHook(LewaHook.LewaHookType.NEW_CLASS)
    static class Injector{
    	 static void addData(ClipData clip,Context context) throws RemoteException{
    		 getService().addData(clip.getItemAt(0).coerceToText(context).toString());
    	 }
    }
    //LEWA ADD END
