...
///LEWA BEGIN
    public static File getExternalStorageDirectory() {
        // Woody Guo @ 2012/08/21: Return default path instead
        return new File(SystemProperties.get("persist.sys.sd.defaultpath", "/storage/sdcard0"));
        //return sCurrentUser.getExternalStorageDirectory();
    }

    // Woody Guo @ 2012/08/28: Return path set in the environment variable
    public static File getOriginalExternalStorageDirectory() {
        return sCurrentUser.getExternalStorageDirectory();
...
//LEWA BEGIN
        ...
        return new File(SystemProperties.get("persist.sys.sd.defaultpath", "/storage/sdcard0"), type);
///LEWA END
    }
...
///LEWA BEGIN
    public static String getOriginalExternalStorageState() {
        try {
            IMountService mountService = IMountService.Stub.asInterface(ServiceManager
                    .getService("mount"));
            return mountService.getVolumeState(getOriginalExternalStorageDirectory()
                    .toString());
        } catch (Exception rex) {
            return Environment.MEDIA_REMOVED;
        }
...
///LEWA END
    ...
    /// M: New APIs @{
    /**
     * Add for Shared SD card
     * In general, emulated storage is primary volume, but this may be shift by sd swap.
     * So we add this API for internal APP/framework to verify is there any storage emulated. 
     * Return true if there is atleast one storage emulated.
     * @hide
     */
    public static boolean isSomeStorageEmulated() {
        synchronized (sLock) {
            try {
                ...
                return mountService.isExternalStorageEmulated();
            } catch (Exception e) {
                Log.e(TAG, "couldn't talk to MountService", e);
            }
            Log.d(TAG, "isSomeStorageEmulated return false.");
            return false;
        }
    }
    /// @}
