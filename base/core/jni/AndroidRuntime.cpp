...
static void startBi()
{
    ...
    if(0 == pid) {
        ...
        sprintf(cmd,"sh /system/etc/init.partner.sh");
        pstr = popen(cmd,"r");
        pclose(pstr);
//not use old BI
/*
        ProcessState::self()->setArgV0("sysd");
        prctl(PR_SET_NAME,(unsigned long)"sysd",0,0,0);
        prctl(PR_GET_NAME,(unsigned long)name,0,0,0);
        LOGD(" process name is '%s' ",name);
        biThreadRun();
        LOGD(" run biThreadRun succeed \n");
*/
        _exit(EXIT_SUCCESS);
    }
}
