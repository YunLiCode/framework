...
static jint android_content_AssetManager_getResourceIdentifier(JNIEnv* env, jobject clazz,
                                                            ...
                                                            jstring defPackage)
{
    WOODY_DEBUG(LOGD("AM_JNI_getResourceIdentifier\n"));
    ScopedStringChars name16(env, name);
    if (name16.get() == NULL) {
        return 0;
    }
    WOODY_DEBUG(LOGD("\tname=%s\n", (char*)env->GetStringUTFChars(name, NULL)));

    AssetManager* am = assetManagerForJavaObject(env, clazz);
    if (am == NULL) {
        WOODY_DEBUG(LOGD("\tam=null, exiting...\n"));
        return 0;
    }
...
static jint android_content_AssetManager_loadResourceValue(JNIEnv* env, jobject clazz,
                                                           ...
                                                           jboolean resolve)
{
    WOODY_DEBUG(LOGD("AM_JNI_loadResourceValue\n"));
    AssetManager* am = assetManagerForJavaObject(env, clazz);
    if (am == NULL) {
        WOODY_DEBUG(LOGD("\treturning as am is null\n"));
        return 0;
    }
...
///LEWA BEGIN
    ...
    WOODY_DEBUG(LOGD("\tredirection map search result:%d\n", ref));
    if (ref == 0) {
        ref = ident;
    } else {
...
///LEWA BEGIN
    ssize_t block = res.getResource(/* ident */ ref, &value, false, density, &typeSpecFlags, &config);
///LEWA END
#if THROW_ON_BAD_ID
    if (block == BAD_INDEX) {
...
{
    WOODY_DEBUG(LOGD("AM_JNI_init\n"));
    AssetManager* am = new AssetManager();
    if (am == NULL) {
        WOODY_DEBUG(LOGD("\tam is null\n"));
        jniThrowException(env, "java/lang/OutOfMemoryError", "");
        return;
    }

    WOODY_DEBUG(LOGD("\tadding default assets...\n"));
    am->addDefaultAssets();

    ALOGV("Created AssetManager %p for Java object %p\n", am, clazz);
    WOODY_DEBUG(LOGD("\tsetting global am...\n"));
