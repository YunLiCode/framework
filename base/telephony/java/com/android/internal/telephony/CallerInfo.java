...
///LEWA BEGIN
    public long smallPhotoResource;
    public Drawable smallCachedPhoto;
    public boolean isSmallCachedPhotoCurrent;
///LEWA END
...
///LEWA BEGIN
        info.smallPhotoResource = 0;
        info.smallCachedPhoto = null;
        info.isSmallCachedPhotoCurrent = false;
///LEWA END
...
///LEWA BEGIN
                ...
                // Look for the contact small photo
                columnIndex = cursor.getColumnIndex(PhoneLookup.PHOTO_ID);
                if (columnIndex != -1) {
                    info.smallPhotoResource = cursor.getLong(columnIndex);
                }
///LEWA END
...
///LEWA BEGIN
        Cursor c = null;
        try {
            if (contactRef.toString().startsWith("content://com.android.contacts/phone_lookup")) {
                if (info.person_id > -1) {
                    c = context.getContentResolver().query(Data.CONTENT_URI,
                             new String[] {Data._ID, Data.MIMETYPE, Note.NOTE, Photo.PHOTO},
                             Data.CONTACT_ID + "=?" + " AND "
                                     + "(" + Data.MIMETYPE + "='" + Note.CONTENT_ITEM_TYPE + "'"
                                     + " OR " + Data.MIMETYPE + "='" + Photo.CONTENT_ITEM_TYPE + "'"
                                     +")",
                             new String[] {String.valueOf(info.person_id )}, null);
                    if (c != null && c.moveToFirst()) {
                        c.moveToPosition(-1);
                        int index;
                        String mimeType;
                        while (c.moveToNext()) {
                            index = c.getColumnIndex(Data.MIMETYPE);
                            if (index != -1) {
                                mimeType = c.getString(index);
                                if (mimeType.equals(Note.CONTENT_ITEM_TYPE)) {
                                    index = c.getColumnIndex(Note.NOTE);
                                    if (index != -1) {
                                        info.note = c.getString(index);
                                    } else {
                                        info.note = "";
                                    } 
                                } else if (mimeType.equals(Photo.CONTENT_ITEM_TYPE)) {
                                    index = c.getColumnIndex(Photo.PHOTO);
                                    if (index != -1) {
                                        byte[] photoData = c.getBlob(index);
                                        Bitmap photoBitmap = null;
                                        photoBitmap = BitmapFactory.decodeByteArray(photoData, 0,
                                                photoData.length, null);
                                        if (photoBitmap != null) {
                                        	info.isSmallCachedPhotoCurrent = true;
                                        	info.smallCachedPhoto = new BitmapDrawable(photoBitmap);
                                        }
                                    }
                                }
                            }
            ...
            Log.w(TAG, "Exception thrown during query", e);
        } finally {
            if (c != null) {
                c.close();
            }
...
///LEWA BEGIN
                    .append("\nphotoResource: " + smallPhotoResource)
///LEWA END
                    .append("\nperson_id: " + person_id)
