...
public class WifiStateMachine extends StateMachine {
   ...
   /* Set the frequency band */
    ...
    /* M: For auto connect */
    private IWifiAutoConnect mWifiAutoConnect;
    ...
    public WifiStateMachine(Context context, String wlanInterface) {
        ...
        mWifiAutoConnect = MediatekClassFactory.createInstance(IWifiAutoConnect.class, context);
        mContext = context;
        ...
        mDefaultFrameworkScanIntervalMs = mWifiAutoConnect.defaultFrameworkScanIntervalMs();
    ...
    public void disconnectCommand() {
        if (mWifiAutoConnect.hasCustomizedAutoConnect()
            && mLastNetworkId != WifiConfiguration.INVALID_NETWORK_ID) {
    ...
    public void enableAllNetworks() {
        if (mWifiAutoConnect.shouldEnableAllNetworksForScreenOn()) {
            ...
            for (String line : lines) {
                if (line.startsWith(BSSID_STR)) {
                    ...
                    if (bssid != null) {
                        ...
                        if(!wifiSsid.toString().startsWith("NVRAM WARNING")) {
              ...
              case DhcpStateMachine.CMD_POST_DHCP_ACTION:
                ...
                case WifiMonitor.NETWORK_CONNECTION_EVENT:
                    ...
                    if (mLastBssid != null && mLastBssid.equals((String)message.obj)) {
                        if (mDhcpStateMachine != null) {
                            Xlog.d(TAG, "Renew IP address for connection event!");
                            mDhcpStateMachine.sendMessage(DhcpStateMachine.CMD_RENEW_DHCP);
                        } else {
                            Xlog.e(TAG, "mDhcpStateMachine is null!");
                        }
                        if (FeatureOption.MTK_DHCPV6C_WIFI) {
                            if (mDhcpV6StateMachine != null) {
                                Xlog.d(TAG, "Renew IPV6 address for connection event!");
                                mDhcpV6StateMachine.sendMessage(DhcpStateMachine.CMD_RENEW_DHCP);
                            } else {
                                Xlog.e(TAG, "mDhcpV6StateMachine is null!");
                            }
                        }
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                case CMD_STOP_AP:
                    ...
                    Xlog.d(TAG, "Close supplicant connection for hotspot!");
    ...
    public boolean isWifiConnecting(int connectingNetworkId) {
        return mWifiAutoConnect.isWifiConnecting(connectingNetworkId, mWifiConfigStore.getDisconnectNetworks())
                || (getCurrentState() == mWpsRunningState);
    }

    public boolean hasConnectableAP() {
        return mWifiAutoConnect.hasConnectableAP();
    }

    public void suspendNotification() {
        mWifiAutoConnect.suspendNotification();
    }

    /* In mWifiAutoConnect.init(), it has mWifiManager = (WifiManager) mContext.getSystemService(Context.WIFI_SERVICE);
    ...
    public void autoConnectInit() {
        mWifiAutoConnect.init();
    }

    public int getSecurity(WifiConfiguration config) {
        return mWifiAutoConnect.getSecurity(config);
    }

    public int getSecurity(ScanResult result){
        return mWifiAutoConnect.getSecurity(result);
    }

    public boolean hasCustomizedAutoConnect() {
        return mWifiAutoConnect.hasCustomizedAutoConnect();
    }
    ...
    private void initializeExtra() {
        ...
        intentFilter.addAction(IWifiAutoConnect.AUTOCONNECT_SETTINGS_CHANGE);
        ...
        BroadcastReceiver receiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (action.equals("com.mtk.beamplus.activated")) {
                ...
                } else if (action.equals(IWifiAutoConnect.AUTOCONNECT_SETTINGS_CHANGE)) {
                    ...
                    Xlog.d(TAG, "Received ACTION_WIFI_DISPLAY_STATUS_CHANGED, state:" + state);
                    if (state == WifiDisplayStatus.DISPLAY_STATE_CONNECTED) {
                        mWfdConnected.set(true);
                    } else if (state == WifiDisplayStatus.DISPLAY_STATE_NOT_CONNECTED) {
    ...
    private void updateAutoConnectSettings() {
        ...
        List<WifiConfiguration> networks = mWifiConfigStore.getConfiguredNetworks();
        if (null != networks) {
            if (mWifiAutoConnect.shouldAutoConnect()) {
                if (!isConnecting) {
    ...
    private int getBestSignalNetworkId() {
        ...
        if (mScanResults == null || networks == null || networks.size() < MIN_NETWORKS_NUM) {
            Xlog.d(TAG,"No scan result or configured networks less than " + MIN_NETWORKS_NUM);
            return networkId;
        }
        for (WifiConfiguration network : networks) {
        ...
        if (rssi > BEST_SIGNAL_THRESHOLD) {
            Xlog.d(TAG, "Found the best AP, networkId:" + networkId
                + ", rssi:" + rssi + ", ssid:" + ssid);
            return networkId;
        } else {
            Xlog.d(TAG, "No suitable AP found.");
    ...
    private void showReselectionDialog() {
        ...
        int priorityType = Settings.System.getInt(mContext.getContentResolver(), Settings.System.WIFI_PRIORITY_TYPE,
                Settings.System.WIFI_PRIORITY_TYPE_DEFAULT);
        Xlog.d(TAG, "showReselectionDialog, reselectType:" + reselectType + ", priorityType:" + priorityType
            + ", mLastNetworkId:" + mLastNetworkId + ", mDisconnectNetworkId:" + mDisconnectNetworkId);
        if (reselectType == Settings.System.WIFI_SELECT_SSID_MANUL) {
            return;
        }
        int networkId = INVALID_NETWORK_ID;
        if (priorityType == Settings.System.WIFI_PRIORITY_TYPE_DEFAULT) {
            networkId = getBestSignalNetworkId();
        } else {
            networkId = getHighPriorityNetworkId();
        }
