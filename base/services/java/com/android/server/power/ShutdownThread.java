...
///LEWA BEGIN
            ...
            if (mReboot) {
                ...
                dialog = new AlertDialog.Builder(context, AlertDialog.THEME_HOLO_LIGHT)
                        ...
                        .setTitle(com.lewa.internal.R.string.reboot_system)
                        .setSingleChoiceItems(com.lewa.internal.R.array.shutdown_reboot_options, 0, new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                if (which < 0)
                                    return;

                                String actions[] = context.getResources().getStringArray(com.lewa.internal.R.array.shutdown_reboot_actions);
...
///LEWA BEGIN
        if(lewa.os.Build.SHUTDOWN_ANIMATIOM == true){
            mShutOffAnimation = true;
        }
///LEWA END
...
///LEWA BEGIN
                    ...
                    pd.setTitle(context.getText(mReboot
                            ? com.lewa.internal.R.string.reboot_reboot
                            : com.android.internal.R.string.power_off));
                    pd.setMessage(context.getText(mReboot
                            ? com.lewa.internal.R.string.reboot_progress
                            : com.android.internal.R.string.shutdown_progress));
///LEWA END
                    pd.setIndeterminate(true);
                    pd.setCancelable(false);
                    pd.getWindow().setType(WindowManager.LayoutParams.TYPE_STATUS_BAR_PANEL);
                    /* To fix video+UI+blur flick issue */
