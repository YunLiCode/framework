    ...
    public void systemReady(TwilightService twilight, DreamManagerService dreamManager) {
        synchronized (mLock) {
            ...
            if (FeatureOption.MTK_IPO_SUPPORT == true) {
                ...
                /// M: poweroff alarm clock snooze can not display screen @{
                /*
                filter = new IntentFilter();
                ...
                mContext.registerReceiver(new BroadcastReceiver() {
                    @Override
                    public void onReceive(Context context, Intent intent) {
                        // receive preboot_ipo intent
                        // if sys.boot_completed not 1, set mBootCompleted as false for receving boot_completed intent and PMS initialization
                        if (mBootCompleted == true && !SystemProperties.get("sys.boot_completed").equals("1")) {
                            Slog.d(TAG, "reset mBootCompleted to false");
...
///LEWA BEGIN
    public void rebootConfirm(String reason , final boolean confirm) {
        mContext.enforceCallingOrSelfPermission(android.Manifest.permission.REBOOT, null);
        ...
        Runnable runnable = new Runnable() {
            public void run() {
                synchronized (this) {
                    ShutdownThread.reboot(mContext, finalReason, confirm);
                }
