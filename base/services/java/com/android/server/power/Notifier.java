...
final class Notifier {
    ...
    public void onGoToSleepStarted(int reason) {
        ...
        synchronized (mLock) {
///LEWA BEGIN
            interceptLockscreenScreenshot();
///LEWA END
            mLastGoToSleepReason = reason;
        }
    }
///LEWA BEGIN
    private java.lang.reflect.Method mBitmapProviderScreenshot;
    //fix for lockscreen screenshot element
    private void interceptLockscreenScreenshot(){
        try {
            if(mBitmapProviderScreenshot == null) {
                ...
                Class<?> bitmapProvider = pkgContext.getClassLoader().loadClass("lewa.laml.elements.BitmapProvider");
                mBitmapProviderScreenshot = bitmapProvider.getMethod("screenshot", Context.class);
            }
            mBitmapProviderScreenshot.invoke(null, mContext);
        } catch (Exception e) {
        }
    }
///LEWA END
