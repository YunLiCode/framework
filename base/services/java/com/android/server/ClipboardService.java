...
///LEWA BEGIN
    ...
    public void addData(String data) throws RemoteException {
        mDatas.add(data); int size = mDatas.size();
        Log.i("ClipExtraService", "add in service:" + data + " saved size=" + size); if (size >= 20) mDatas.subList(0, size - 3).clear();
    }

    /**@hide */
    @LewaHook(LewaHook.LewaHookType.NEW_METHOD)
    public List<String> getData(int number) throws RemoteException {
        int size = mDatas.size();
        if (size < number) {
            return mDatas;
        }
        return mDatas.subList(size - number, size);
    }
///LEWA END
}
