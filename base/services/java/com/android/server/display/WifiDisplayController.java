...
final class WifiDisplayController implements DumpUtils.Dump {
    ...
    private void updateSettings() {
        ...
        if (true==mWifiDisplayOnSetting && true==mBTOnSetting) {
            dialogWfdBtConflict(WFDCONTROLLER_WFD_UPDATE, true, false);
        } else {
    ...
    public void requestConnect(String address) {
        ...
        if (mIsConnected_OtherP2p) {
            ...
            AlertDialog dialog = new AlertDialog.Builder(mContext,AlertDialog.THEME_HOLO_LIGHT)
    ...
    private void disconnect() {
        ...
        updateWfdStatFile(WFDCONTROLLER_WFD_STAT_DISCONNECT);
        if (null != mRemoteDisplayInterface) {
            updateConnection();
        }
        ///@}
    }
    ...
    private void updateConnection() {
        ...
        ///M:@{
        if (mRemoteDisplay != null && mConnectedDevice != mDesiredDevice && null!=mRemoteDisplayInterface) {
        ...
        if (mConnectedDevice == null && mConnectingDevice == null) {
            ...
            config.wps.setup = loadWfdWpsSetup();  ///Add by MTK
    ...
    private void handleConnectionChanged(NetworkInfo networkInfo) {
        ...
        if (null == mDesiredDevice) {
            ...
            Slog.w(TAG, "Wifi P2p connection is " + mIsConnected_OtherP2p + ", but it does not wifidisplay trigger");
        }
        ///@}
    }
        ...
        if (!Objects.equal(mAdvertisedDisplay, display)
            ...
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (oldDisplay != null && false == mMR0DataPathConnected) {
                    ///@}
    ...
    private void updateSettingsBT() {
        ...
        if (true==mBTOnSetting && true==mWifiDisplayOnSetting) {
            if (WifiDisplayStatus.FEATURE_STATE_ON == computeFeatureState()) {
                dialogWfdBtConflict(WFDCONTROLLER_BT_UPDATE, false, true);
    ...
    private void dialogWfdBtConflict(int which, boolean wfd, boolean bt) {
        if (DEBUG) {
        ...
        if (WFDCONTROLLER_WFD_UPDATE == which) {
            AlertDialog dialog = new AlertDialog.Builder(mContext,AlertDialog.THEME_HOLO_LIGHT)
                .setMessage(r.getString(com.mediatek.internal.R.string.wifi_display_bt_off_wfd_on))
                .setPositiveButton(r.getString(R.string.dlg_ok), new OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            if (DEBUG) {
                                Slog.d(TAG, "WifiDisplay on, user turn off BT");
                            }
