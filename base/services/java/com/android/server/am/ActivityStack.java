...
final class ActivityStack {
    ...
    private static boolean DEBUG_STATUSBAR = false;
    /// @}
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case HALT_RESUMING_TIMEOUT_MSG: {
                    Slog.d(TAG,"WMS runs the animation too long");
                ...
                case RESTORE_RESUMING_MSG: {
                    Slog.d(TAG,"No animation. Resume the top activity");
                    synchronized (mService) {
                        resumeTopActivityLocked(mAnimationPrev,mAnimationOptions);
                    }
                } break;
...
///LEWA BEGIN
        //add by lewa zhangxianjia, for statusbar change background
...
///LEWA END
    ...
    public String mBgRunpkg = "com.android.launcher";
...
///LEWA BEGIN
///ADD BY ZHANGXIANJIA
    public void makeStatusbarNone(ActivityRecord next) {
            if(true){
                 mHandler.removeCallbacks(bgRunnable);
            try {
                if(ibs == null) {
                    ibs = StatusBarManager.getService();
                }
                ibs.setStatusbarBlackBg();
            }catch (Exception ex) {
                ex.printStackTrace();
            }
            }else if(next != null && next.isHomeActivity) {
                mHandler.post(bgRunnable);
            }
    }
    ...
    public List<String> getHomes() {
        ...
        for(ResolveInfo ri : resolveInfo) {
            ...
            if(DEBUG_STATUSBAR)
            Log.d("zhangxianjia", "home activitys add + " + ri.activityInfo.name);
        }
        return names;
    }
    ...
    public boolean isHome(){
        ...
        return getHomes().contains(rti.get(0).topActivity.getClassName());
    }
    ...
    private boolean isCurrentHomeActivity(ComponentName component, ActivityInfo homeInfo) {
        ...
        if(DEBUG_STATUSBAR)
        Log.d("zhangxianjia", "current homeactivity = " + homeInfo.name + "      cureent actvity = " + component.getClassName());
        return homeInfo != null
                && homeInfo.packageName.equals(component.getPackageName())
                && homeInfo.name.equals(component.getClassName());
    }

    public void makeStautsbarTrans(ActivityRecord prev) {
        if(mMainStack) {
            if(getHomes().contains(mTargetActivity)){
                if (!mHandler.hasCallbacks(bgRunnable)) {
                    if (null == mLockscreens) {
                        try {
                            mLockscreens = mContext.getResources().getStringArray(
                                    com.lewa.internal.R.array.config_standaloneLockscreens);
                        } catch (Exception e) {
                        }
                        if (null != mLockscreens && 1 < mLockscreens.length) {
                            Arrays.sort(mLockscreens);
                        }
                    }
                    //mBgRunpkg = packagenm;
                    mHandler.post(bgRunnable);
...
///LEWA END
    ...
    final boolean resumeTopActivityLocked(ActivityRecord prev, Bundle options) {
        ...
        if(next != null) {
            if(DEBUG_STATUSBAR)
                Log.d("zhangxianjia", "resumeTopActivityLocked   " + next.info.name);
            mTargetActivity = next.info.name;
        }
        if(prev != null && prev.isHomeActivity){
            makeStatusbarNone(next);
        }
        ///LEWA END
        ...
        synchronized (mHaltActivityLock) {
            if (mHaltActivity) {
                Slog.d(TAG, "Because running the window animation, can't resumeTopActivityLocked "
                    + mAnimationPrev);
                mAnimationPrev = prev;
                mAnimationOptions = options;
                mActivityReumePending = true;
                return false;
            }
        }
        /// @}
    ...
    private final void startActivityLocked(ActivityRecord r, boolean newTask,
        ...
        int addPos = -1;
        mTargetActivity = r.info.name;
        if(DEBUG_STATUSBAR)
        Log.d("zhangxianjia", "startActivityLocked" + r.info.name);
        if (!newTask) {
    ...
    private final ActivityRecord finishCurrentActivityLocked(ActivityRecord r,
        ...
        /// M: finish activity directly (ALPS00070338) @{
        else if (mode == FINISH_AFTER_PAUSE
                && prevState == ActivityState.PAUSED
                && (r.intent.getFlags()&Intent.FLAG_ACTIVITY_NO_HISTORY) != 0) {
            Slog.i(TAG, "finish the activity - FLAG_ACTIVITY_NO_HISTORY");
            return destroyActivityLocked(r, true, true, "finish-after-pause") ? null : r;
        }
        /// @}
        else {
    ...
    boolean mActivityReumePending = false;
...
*/
    ...
    * @return true: Resume the top activity. Otherwise,
    *                     it returns false.
    */
    public boolean restoreActivityResuming() {
    /*
        synchronized (mHaltActivityLock) {
            Slog.d(TAG, "restoreActivityResuming mHaltActivity=" +mHaltActivity
                + ", mActivityReumePending = " + mActivityReumePending);
            if (!mHaltActivity) {
                return false;
            }
            mHaltActivity = false;

            if (!mActivityReumePending) {
                return false;
            }
            mActivityReumePending = false;
