...
///LEWA BEGIN
            if(DEBUG_STATUSBAR)
            Log.d("zhangxianjia", "window visible = " + info.name);
            if(!isHomeActivity) {
                stack.makeStatusbarNone(this);
            }else {
                stack.makeStautsbarTrans(this);
            }
...
///LEWA END
    ...
    ///LEWA BEGIN
        if(DEBUG_STATUSBAR)
        Log.d("zhangxianjia", "window gone = " + info.name);
        if(!stack.getHomes().contains(info.name)) {
            stack.makeStautsbarTrans(this);
