...
class MountService extends IMountService.Stub
    ...
    private boolean                               mCheckedSDExist = false;
    ...
    private void updatePublicVolumeState(StorageVolume volume, String state) {
        ...
        // when not emulated.
        if ((volume.isPrimary() && !volume.isEmulated() || (FeatureOption.MTK_2SDCARD_SWAP && isSDExist()))) {
    ...
    public boolean onEvent(int code, String raw, String[] cooked) {
        ...
        if (code == VoldResponseCode.VolumeStateChange) {
            ...
            intent.putExtra(SD_EXIST, updateSDExist());
    ...
    private void updateDefaultpath() {
        ...
        if (!Environment.MEDIA_MOUNTED.equals(getVolumeState(defaultPath))) {
            ...
            if (needChange && (!path.equals(defaultPath))) {
                ...
                StorageManager.setDefaultPath(path);
            }
        }
    }
    /**
     ...
     * Depricated
     */
    private void updateEmulatedMounted() {
    /*
        synchronized (mVolumes) {
            int size = mVolumes.size();            
            for (int i = 0; i < size; i++) {
                ...
                if (volume.isEmulated()) {
                    //updatePublicVolumeState(volume.getPath(), Environment.MEDIA_MOUNTED);
                    notifyVolumeStateChange(null, volume.getPath(), 
                            VolumeState.NoMedia, VolumeState.Mounted);
                    
                }
                volume.setStorageId(i);						
                Slog.d(TAG, 
                    "Storage[" + i + "] path: " + volume.getPath() + 
     ...
     * SD swap
     */
    private boolean doGetSDExist() {
        String sdExist = PROP_SD_SWAP_FALSE;
        
        try {
            sdExist = SystemProperties.get(PROP_SD_SWAP, PROP_SD_SWAP_FALSE);
        } catch (IllegalArgumentException e) {
            Slog.e(TAG, "IllegalArgumentException when get sdExist:" + e);
        }
        Slog.i(TAG, "doGetSDExist = " + sdExist);
        return sdExist.equals(PROP_SD_SWAP_TRUE);

        /*
        ArrayList<String> rsp;
        try {
            rsp = mConnector.doCommand("volume is_2sd_swapped");
        } catch (NativeDaemonConnectorException ex) {
            Slog.e(TAG, "Failed to determine whether External SD exist.");
            return false;
        }
        ...
        for (String line : rsp) {
            String[] tok = line.split(" ");
            if (tok.length < 1) {
                Slog.e(TAG, "Malformed response to doGetSDExist");
                return false;
            }
    
            int code;
            try {
                code = Integer.parseInt(tok[tok.length - 1]);
            } catch (NumberFormatException nfe) {
                Slog.e(TAG, String.format("Error parsing code %s", tok[tok.length - 1]));
                return false;
            }
            if (code == 1) {
                return true;
    ...
    private void doSDSwapVolumeUpdate() {
        ...
        // updateSDExist() will get is there swap from vold
        // we will update mVolumes accord to updateSDExist()
		...
		boolean  sdSwapStatus = updateSDExist();
    ...
    public boolean isSDExist() {
        ...
        Slog.d(TAG, "isSDExist " + mIsSDExist);
        return mIsSDExist;
    }
    ...
    private void updateDefaultPathForSwap(boolean mount, String path) {
        ...
        if (mount && EXTERNAL_SD2.equals(path)) {
            ...
            StorageManager.setDefaultPath(EXTERNAL_SD2);
        } else if (!mount) {
            ...
            if (EXTERNAL_SD1.equals(defaultPath)) {
                ...
                StorageManager.setDefaultPath(EXTERNAL_SD1);
            }
        }
    }
    ...
    /**
     * SD swap
     * MountService should always call this to update mIsSDExist
     */
    private boolean updateSDExist() {
        mIsSDExist = doGetSDExist();
        mCheckedSDExist = true;
        return mIsSDExist;
    }
    /// @}
}
