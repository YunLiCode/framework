...
class TelephonyRegistry extends ITelephonyRegistry.Stub {
    ...
    //MTK-START [mtk04070][111128][ALPS00093395]Added for Gemini
    TelephonyRegistry(Context context, boolean isGemini) {
        this(context);
        if(isGemini){
        mySimId = PhoneConstants.GEMINI_SIM_2; // Overwrite mySimId when Gemini enabled
        }else{
            mySimId = PhoneConstants.GEMINI_SIM_1; // Overwrite mySimId when Gemini enabled
        }
    }
    //MTK-END [mtk04070][111128][ALPS00093395]Added for Gemini
...
///LEWA BEGIN
    private boolean checkPermission(int events){
        if ((events & PHONE_STATE_PERMISSION_MASK) != 0)
            return lewa.content.PermissionHelper.checkPermission(mContext, getCallingPid(), getCallingUid(), lewa.content.PermissionHelper.PERMISSION_READ_PHONE_STATE);
        if ((events & PhoneStateListener.LISTEN_CELL_LOCATION) != 0 || (events & PhoneStateListener.LISTEN_CELL_INFO) != 0)
            return lewa.content.PermissionHelper.checkPermission(mContext, getCallingPid(), getCallingUid(), lewa.content.PermissionHelper.PERMISSION_ACCESS_LOCATION); 
        return true;
    }
///LEWA END
}
